C     ******************************************************************
C     FARM PROCESS VERSION 2.1 (FMP2) FOR MODFLOW-2005 VERSION 1.6_01
C     09/18/2009
C     ******************************************************************
C
...
C-----CONSTANTS
C       integer constants:
C        MAIN:         IN,IUNITSFR,IUNITMNW,IUNITUZF,IGRID,IUNITPCG,
...
C-------CONSTANTS
     ...
     4  IRDFL,ISRRFL,IRRFL,IALLOT,IFWLCB,IFNRCB,ISDPFL,IOPFL,IPAPFL,
     5  IFBPFL,IPFWEL,IWELLFLD,QBD,MCLOSE,IRTPFL,
     6  MXFWEL,NFWELS,NFWLVL,IFWLAL,IFWLPB,NNPFWL,LENSIM,NAUX
        DOUBLE PRECISION,SAVE,POINTER:: PCLOSE,ALLOT,QCLOSE,HPCT,RPCT   !FORMERLY REAL
C
C-------ONE-DIMENSIONAL VARIABLES
        CHARACTER(LEN=16),SAVE,DIMENSION(:),POINTER:: FWLAUX
        INTEGER,SAVE,DIMENSION(:),POINTER:: NWPERF,KNTR,NFSEG,IRDRFL
        DOUBLE PRECISION,SAVE,DIMENSION(:),POINTER::                    !FORMERLY REAL
     1  QAVF,QDEF,QEXC,HLIFT,ELHL,SFRADD,FDSEGL,FCSEGL,RDR,DIVADD,DIVTMP
        DOUBLE PRECISION,SAVE,DIMENSION(:),POINTER::
     1  QMAXF,QREQ,QREQOLD,TFDR,TFDROLD,FLOWINMIN,QSTRMIN
C
C-------TWO-DIMENSIONAL VARIABLES
        INTEGER,SAVE,DIMENSION(:,:),POINTER::
     1  IFID,ICID,ISID,IFALLOW,IFDRID,IFCRID,ISRD,ISRR  
C        REAL, SAVE, DIMENSION (:,:),POINTER:: GSURFR,ETRR,PFLXR         !HAVE TO REMAIN REAL WHEN USING U2DREL
        DOUBLE PRECISION,SAVE,DIMENSION(:,:),POINTER::                  !FORMERLY REAL
     1  FWELL,GSURF,CU,CU2D,ETR,ROOT,RT2D,PFLX,PFLR,TPPOT,EPPOT,EGW,TGW,
     2  DWE,ETC,RTD,DNR,ELNR
        DOUBLE PRECISION,SAVE,DIMENSION(:,:),POINTER::
...
C
C-------THREE-DIMENSIONAL VARIABLES
        DOUBLE PRECISION,SAVE,DIMENSION(:,:,:),POINTER:: SOIL2D,RNRD
C
      TYPE FMPTYPE
C
C-------CONSTANTS
     ...
     5  IFBPFL,IPFWEL,IWELLFLD,QBD,MCLOSE,IRTPFL,
     6  MXFWEL,NFWELS,NFWLVL,IFWLAL,IFWLPB,NNPFWL,LENSIM,NAUX
        DOUBLE PRECISION,POINTER:: PCLOSE,ALLOT,QCLOSE,HPCT,RPCT        !FORMERLY REAL
C
C-------ONE-DIMENSIONAL VARIABLES
        CHARACTER(LEN=16),DIMENSION(:),POINTER:: FWLAUX
        INTEGER,DIMENSION(:),POINTER:: NWPERF,KNTR,NFSEG,IRDRFL
        DOUBLE PRECISION,DIMENSION(:),POINTER::                         !FORMERLY REAL
     1  QAVF,QDEF,QEXC,HLIFT,ELHL,SFRADD,FDSEGL,FCSEGL,RDR,DIVADD,DIVTMP
...
C-------TWO-DIMENSIONAL VARIABLES
     ...
     1  IFID,ICID,ISID,IFALLOW,IFDRID,IFCRID,ISRD,ISRR  
C        REAL, DIMENSION (:,:),POINTER:: GSURFR,ETRR,PFLXR               !HAVE TO REMAIN REAL WHEN USING U2DREL
        DOUBLE PRECISION,DIMENSION(:,:),POINTER::                       !FORMERLY REAL
     1  FWELL,GSURF,CU,CU2D,ETR,ROOT,RT2D,PFLX,PFLR,TPPOT,EPPOT,EGW,TGW,
     2  DWE,ETC,RTD,DNR,ELNR
        DOUBLE PRECISION,DIMENSION(:,:),POINTER::
...
C
C-------THREE-DIMENSIONAL VARIABLES
        DOUBLE PRECISION,DIMENSION(:,:,:),POINTER:: SOIL2D,RNRD
C
      END TYPE
      TYPE(FMPTYPE), SAVE:: FMPDAT(10)
C=======================================================================
...
C
C
      SUBROUTINE FMP2AR(IN,IUNITSFR,IUNITMNW,IUNITUZF,IGRID)
C-----VERSION 2 09/18/2009 FMP2AR
C     ******************************************************************
...
C     ------------------------------------------------------------------
     ...
     1                       REJ_INF
      USE GWFMNWMODULE, ONLY:WELL2
      USE GWFBASMODULE, ONLY:IBDOPT
      IMPLICIT NONE
C     ------------------------------------------------------------------
C        ARGUMENTS:
C     ------------------------------------------------------------------
      INTEGER IN,IUNITSFR,IUNITMNW,IUNITUZF,IGRID
C     ------------------------------------------------------------------
C        LOCAL VARIABLES:
      INTEGER IVAR(11)
      CHARACTER*200 LINE
      CHARACTER*46 ANAME(11)
      CHARACTER*7 FLNAM(11)
      CHARACTER*6 SWPR
      CHARACTER*8 FBTYPE
      ...
      INTEGER LLOC,ISTART,ISTOP,N,I,ICB,DATE_TIME(8)
      REAL R,SIMLEN 
C     ------------------------------------------------------------------
C
C     ALLOCATE SPACE FOR CONSTANTS:
     ...
     5  IFBPFL,IPFWEL,IWELLFLD,QBD,MCLOSE,IRTPFL,
     6  MXFWEL,NFWELS,NFWLVL,IFWLAL,IFWLPB,NNPFWL,LENSIM,NAUX)
      ALLOCATE(PCLOSE,ALLOT,QCLOSE,HPCT,RPCT)
C
C1===== IDENTIFY PACKAGE AND INITIALIZE NFWELS ==============================================================
    ...
    1 FORMAT(//,1X,'FMP2 -- FARM PROCESS, VERSION 2.X, ',
...
C                                 Maximum number of parameter farm wells      
      CALL UPARLSTAL(IN,IOUT,LINE,NPFWL,MXPFW)
C
...
C                                 Surface-water allotment (optional: closure criterion for simulated diversions from a river into a diversion-segment (canal))
      ...
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,IALLOT,R,IOUT,IN)
      IF(IALLOT.GT.1) THEN
...
C
      DO I=1,4
      IF((I.EQ.1.AND.(IVAR(I).LT.1.OR.IVAR(I).GT.3)).
     1OR.(I.EQ.2.AND.(IVAR(I).LT.-1.OR.IVAR(I).GT.3.OR.IVAR(I).EQ.0)).
     2OR.(I.EQ.4.AND.(IVAR(I).LT.2.OR.IVAR(I).GT.3))) THEN
      WRITE(IOUT,21) ANAME(I),FLNAM(I),FLNAM(I)
...
C     
      DO I=5,11
      IF((I.EQ.6.OR.I.EQ.10.OR.I.EQ.11).AND.
     1(IVAR(I).LT.0.OR.IVAR(I).GT.2)) THEN
      WRITE(IOUT,23) ANAME(I),FLNAM(I),FLNAM(I),FLNAM(I)
   23 FORMAT(1X,'INPUT-ERROR:',/,1X,A46,
     1' MUST BE SPECIFIED FOR EACH STRESS PERIOD (',A7,'= 2),',/,
     248X,'OR FOR EACH SIMULATION (',A7,'= 1)',/,
     348X,'OR MUST BE ZERO (',A7,'= 0)')
      STOP
      ELSEIF((I.NE.6.AND.I.NE.10.AND.I.NE.11).AND.
     1(IVAR(I).LT.1.OR.IVAR(I).GT.2))THEN
      WRITE(IOUT,22) ANAME(I),FLNAM(I),FLNAM(I)
...
C3B2-----FOR ALL REQUIRED "WHEN-TO-READ" FLAGS, PRINT MESSAGE
      DO I=1,7
      IF(I.EQ.2.AND.IVAR(I).EQ.1) THEN
    ...
    5 IF(IVAR(I).EQ.2) WRITE(IOUT,7) ANAME(I)
    7 FORMAT(3X,A46,' SPECIFIED FOR EACH STRESS PERIOD')
      IF(IVAR(I).EQ.3) WRITE(IOUT,8) ANAME(I)
    8 FORMAT(3X,A46,
...
C
C3C1B---PRINT MESSAGE
      IF((IEBFL.EQ.0.OR.IEBFL.EQ.1).AND.IDEFFL.EQ.0)
     1WRITE(IOUT,51) "EFFICIENCY BEHAVIOR: "
   51 FORMAT(3X,A21,
     1'CONSERVATIVE BEHAVIOR: CONSTANT EFFICIENCY OVER A TIME STEP')
      IF((IEBFL.EQ.2.OR.IEBFL.EQ.3).AND.IDEFFL.EQ.0)
     1WRITE(IOUT,52) "EFFICIENCY BEHAVIOR: "
   52 FORMAT(3X,A21,
...
C     EFFICIENCY DEPENDS ON REDUCED DELIVERY AS A RESULT OF DEFICIT IRRIGATION
      IF(IDEFFL.EQ.-1)
     1WRITE(IOUT,53) "EFFICIENCY BEHAVIOR: "
   53 FORMAT(3X,A21,
...
C3D-----CONSUMPTIVE USE FLAG
  ...
  952 FORMAT(/,1X,
     1'CONSUMPTIVE USE CONCEPT 2: Tgw-act-max = Tc-act-max = Tpot')
      IF(ICCFL.EQ.1.OR.ICCFL.EQ.3) WRITE(IOUT,953)
  953 FORMAT(/,1X,
     ...
     4'APPROXIMATED BY CROP- & SOIL-TYPE SPECIFIC ANALYTICAL SOLUTION)')
      IF(ICCFL.LT.1.OR.ICCFL.GT.4) THEN
      WRITE(IOUT,'(/,1X,
     1"CROP CONSUMPTIVE USE FLAG MUST BE 1,2,3,OR 4!")')
      STOP
      ENDIF
C
...
C3E1----NON-ROUTED SURFACE-WATER DELIVERY
  ...
  101 FORMAT(3X,'NO NON-ROUTED SURFACE-WATER DELIVERIES EXIST')
      IF(INRDFL.EQ.1) WRITE(IOUT,102) MXNRDT
  102 FORMAT(3X,'FARM DELIVERY REQUIREMENT SUPPLIED BY:',
     ...
     2/,5X,'MAXIMUM NUMBER OF NON-ROUTED DELIVERY TYPES = ',I4)
      IF(INRDFL.LT.0.OR.INRDFL.GT.1) THEN
      WRITE(IOUT,*) "NON-ROUTED SW-DELIVERY FLAG MUST BE 0 OR 1!"
      STOP
      ENDIF
C
...
C3E5----FULLY ROUTED SURFACE-WATER RUNOFF-RETURNFLOW
  ...
  117     FORMAT(1X,'INPUT-ERROR: FULLY ROUTED RUNOFF-RETURNFLOW',/,14X,
     ...
     2    /,14X,'==> SPECIFY SFR PACKAGE IN NAME FILE!')
          STOP
        ELSEIF(IUNITSFR.GT.0) THEN
          IF(IRRFL.LT.-1.OR.IRDFL.GT.1)THEN
            WRITE(IOUT,*) "ROUTED SW-DELIVERY FLAG MUST BE -1 OR 1!"
            STOP
          ENDIF
          IF(ISRRFL.EQ.0) THEN
            WRITE(IOUT,118)
...
C3E6----APPLIED SURFACE-WATER RIGHT
      IF(IALLOT.GT.0.AND.IUNITSFR.EQ.0) THEN
      WRITE(IOUT,9401)
 9401 FORMAT(1X,'INPUT-ERROR: SURFACE-WATER DELIVERY CONSTRAINTS',/,
     114X,'CANNOT OCCUR IF SFR1 PACKAGE IS NOT SPECIFIED IN NAME FILE.',
     2/,14X,'==> SELECT IALLOT=0 OR SPECIFY SFR1 PACKAGE IN NAME FILE!')
      STOP
      ENDIF
      IF(IALLOT.GT.0.AND.IRDFL.EQ.0.AND.ISRDFL.EQ.0) THEN
      WRITE(IOUT,9402)
 9402 FORMAT(1X,'INPUT-ERROR: SURFACE-WATER DELIVERY CONSTRAINTS',/,
     ...
     2DO NOT EXIST.',/,14X,
     3'==> SELECT IALLOT=0 OR SELECT IRDFL=0 OR ISRDFL=1,OR=2!')  
      STOP
      ENDIF
      IF(IALLOT.EQ.1) WRITE(IOUT,950)
  950 FORMAT(3X,'EQUAL APPROPRIATION ALLOTMENT HEIGHT SELECTED')
      IF(IALLOT.GE.2) WRITE(IOUT,960) PCLOSE
  960 FORMAT(3X,'PRIOR APPROPRIATION SELECTED; FLOW RESIDUAL = ',E11.4)
      IF(IALLOT.EQ.2) WRITE(IOUT,970)
  970 FORMAT(3X,'WATER RIGHTS CALLS ARE READ FOR EACH STRESS PERIOD.')
C
C
...
C3F2C---TO BINARY FILE
   ...
   63 FORMAT(3X,'FARM WELL FLOW RATES:',/,5X,     
     1'A LIST (NODE, RATE) WILL BE SAVED AS BINARY FILE ON UNIT',I3,/,5X
   ...
   64 FORMAT(3X,'FARM WELL FLOW RATES:',/,5X,     
     ...
     2 I3,/,5X,'FOR TIME STEPS, FOR WHICH IN OUTPUT CONTOL',/,5X,
     3'"SAVE BUDGET" IS SPECIFIED (USING WORDS),',
     4' OR ICBCFL IS NOT 0 (USING NUMERIC CODES)')
C
...
C3F3C---TO BINARY FILE
   ...
   77 FORMAT(3X,'FARM NET RECHARGE FLOW RATES:',/,5X,     
     ...
     3'WILL BE SAVED AS BINARY FILE ON UNIT',I3,
...
C3F4C---TO BINARY FILE
   ...
   84 FORMAT(3X,'FARM DEMAND & SUPPLY FLOW RATES:',/,5X,
     ...
     2'FINAL DEMAND AND SUPPLY FLOW RATES AFTER ',
     3'THE APPLICATION OF A DEFICIENCY SCENARIO',/,5X,
     4'WILL BE SAVED AS BINARY FILE ON UNIT',I3' FOR TIME STEPS, ',
...
C3F5B---TO ASCII FILE
  ...
  202 FORMAT(6X,'LIST = (PER,TSTP,TIME,FID;',/,6X,'RATES INTO FARM: ',
     ...
     &'CUMULATIVE VOLUMES INTO FARM: ',
     &'V-p-in, V-sw-in, V-gw-in, V-ext-in, V-tot-in,',/,6X,
     &'CUMULATIVE VOLUMUES OUT OF FARM: ',
     &'V-et-out,V-ineff-out, V-sw-out, V-gw-out, V-tot-out,',/,6X,
     &'VOLUMETRIC BUDGET ERROR: V-in-minus-out, V-Discrepancy [%])')
      ENDIF
c
      IF(IFBPFL.EQ.2) THEN
      FBTYPE="DETAILED"
...
C3F6A---TO LIST FILE
      IF(IUNITSFR.GT.0) THEN     
      IF(IRTPFL.EQ.-2) WRITE(IOUT,86) 'ROUTING INFOMATION:' 
   86 FORMAT(3X,A19,/,5X,'WRITTEN TO LISTING FILE FOR THE ',
     1'FIRST STRESS PERIOD ONLY')
      IF(IRTPFL.EQ.-1) WRITE(IOUT,87) 'ROUTING INFOMATION:'
   87 FORMAT(3X,A19,/,5X,'WRITTEN TO LISTING FILE FOR EVERY ',
     1'STRESS PERIOD') 
      IF(IRTPFL.EQ.0) WRITE(IOUT,88) 'ROUTING INFOMATION:'
   88 FORMAT(3X,A19,/,5X,'NOT WRITTEN ')
      IF(IRTPFL.EQ.1) WRITE(IOUT,89) 'ROUTING INFOMATION:'
   89 FORMAT(3X,A19,/,5X,'WRITTEN TO ASCII FILE "ROUT.OUT" ',
     1'FOR EVERY STRESS PERIOD')
      IF(IRTPFL.EQ.2) WRITE(IOUT,891) 'ROUTING INFOMATION:'
  891 FORMAT(3X,A19,/,5X,'WRITTEN TO ASCII FILE "ROUT.OUT" ',
     1'FOR FIRST STRESS PERIOD ONLY')    
      ENDIF
C
...
C-------Allocate space in the double precision array for the Farm Well list and initialize
      ...
      FWELL=0.D0
C     (The FMP1 rule 'Deactivating the FARM PROCESS because MXACTFW=0' was dropped in FMP2)
C
...
c       so that it can be amended in FMP2FM by:
...
C5A-----Allocate space for the integer array for the NWPERF and KNTR lists and initialize
      ALLOCATE(NWPERF(NFARMS),KNTR(NFARMS))
      NWPERF=0
      KNTR=0
C5B-----Allocate space for the integer arrays for the FID, CID, and SID arrays and initialize
      ALLOCATE(IFID(NCOL,NROW),ICID(NCOL,NROW),ISID(NCOL,NROW))
      IFID=0
      ICID=0
      ISID=0
...
C           (ISRDFL.EQ.0.OR.(ISRDFL.NE.0.AND.ISRD(2,NF).EQ.0.AND.ISRD(3,NF).EQ.0.AND.ISRD(4,NF).EQ.0.AND.ISRD(5,NF).EQ.0))
...
C
C
      SUBROUTINE FMP2RQ(IN,IUNITSFR,IUNITMNW,IGRID)
C-----VERSION 2 09/18/2009 FMP2RQ
C     ******************************************************************
...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE FMPMODULE,    ONLY:NFWLVL,IFWLAL,NPFWL,FWELL,IFWLPB,MXFWEL,
     1    IPFWEL,NFARMS,IFID,EFF,EF2D,ICID,GSURF,SOIL,ISID,SOIL2D,PSI,
     2    IFALLOW,CROPBEN,WATERCOST,ROOT,RT2D,IRTFL,ICUFL,IPFL,ICCFL,
     3    IFTEFL,IIESWFL,IEFFL,IEBFL,IROTFL,IDEFFL,IBEN,ICOST,ISRDFL,
     4    ISRRFL,NCROPS,NSOILS,CROPLIST,CU,LENSIM,CLIMATE,ETC,RTD,
     5    FIESW,FTE,RISERUN,ISRD,ISRR,FWLAUX,NAUX
c      USE FMPMODULE,    ONLY: GSURFR    
      USE GLOBAL,       ONLY:IOUT,NCOL,NROW,NLAY,DELR,DELC
      IMPLICIT NONE
C     ------------------------------------------------------------------
C        ARGUMENTS:
C     ------------------------------------------------------------------
      INTEGER IGRID,IN,IUNITMNW,IUNITSFR
C     ------------------------------------------------------------------
C        LOCAL VARIABLES:
c     ------------------------------------------------------------------
     ...
     1 IEXITFLAG,IR,IC,IR1,IR2,IR3,IC1,IC2,IC3,IMX,IMY,ID,L
...
C1B-----READ PARAMETER WITHOUT INSTANCES.
            CALL FMP2WELRD(NLST,FWELL,LSTBEG,NFWLVL,MXFWEL,IFWLAL+1,IN,
     1         IOUT,
     2'WELL NO. LAYER    ROW    COL  F-WELL ID    FARM ID      QMAXfac',
     3         FWLAUX,5,NAUX,NCOL,NROW,NLAY,6,6,IPFWEL,IUNITMNW)
          ELSE
C1C-----READ INSTANCES.
            NINLST=NLST/NUMINST
            DO 10 I=1,NUMINST
...
C1D-----READ AND PRINT A LIST OF ATTRIBUTES ASSOCIATED WITH PARAMETER-FARM-WELLS 
            CALL FMP2WELRD(NINLST,FWELL,LSTBEG,NFWLVL,MXFWEL,IFWLAL+1,
     1        IN,IOUT,
     2'WELL NO. LAYER    ROW    COL  F-WELL ID    FARM ID      QMAXfac',
     3        FWLAUX,5,NAUX,NCOL,NROW,NLAY,6,6,IPFWEL,IUNITMNW)
...
C      CALL U2DREL(GSURFR,ANAME(3),NROW,NCOL,0,IN,IOUT)      
c      GSURF=DBLE(GSURFR)
C
...
C3B-----INITIALIZE AND READ FARM-SPECIFIC EFFICIENCY LIST/MATRIX 
      IF(IEFFL.EQ.1) THEN 
        ...
        ENDDO
        ENDDO
        CALL FMP2DPLSTRD(EFF,NCROPS+3,NFARMS,IN,IOUT,2,NCROPS+1)
      ENDIF
C
...
C                         ANALYTICAL SOLUTIONS FOR THE RESPECTIVE SOILTYPE IN FM-ROUTINE      
      CALL FMP2DPWDLSTRD(SOIL,SOILTYPE,7,NSOILS,IN,IOUT,2,2)
      IF(ICCFL.EQ.1.OR.ICCFL.EQ.3) THEN
      DO NS=1,NSOILS
      do n=1,7
      enddo
...
C4C-----CHECK FOR EXCLUSIONS AND STOP PROGRAM 
         DO IR=1,NROW
         DO IC=1,NCOL 
            IF(ISID(IC,IR).EQ.0.AND.IFID(IC,IR).NE.0) THEN
...
C       (ONE LUMPED VALUE PER ENTIRE SIMULATION)
      IF(IFTEFL.EQ.1) THEN
      CALL FMP2DPLSTRD(FTE,4,NCROPS,IN,IOUT,2,4)
      write(iout,103)
  103 format(1x,/
...
C5D1----READ FRACTIONS OF IN-EFFICIENT LOSSES TO SW-RUNOFF FROM PRECIPITATION AND FROM IRRIGATION
      IF(IIESWFL.EQ.1) THEN
      CALL FMP2DPLSTRD(FIESW,3,NCROPS,IN,IOUT,2,3)
      write(iout,106)
  106 format(1x,/
...
C6A-----READ CLIMATE LIST
      CALL FMP2DPLSTRD(CLIMATE,5,LENSIM,IN,IOUT,0,0)
C
...
C       FALLOW(2,NC): FALLOW FLAG (1- CAN GO FALLOW / 0- CAN'T GO FALLOW)
      IF(IDEFFL.EQ.-2) THEN
      CALL FMP2INTLSTRD(IFALLOW,2,NCROPS,IN,IOUT) 
      ENDIF
C
...
C       CROPBEN(4,NC): CROP MARKET PRICE [$/MASS]
        IF(IBEN.EQ.1) CALL FMP2DPLSTRD(CROPBEN,4,NCROPS,IN,IOUT,0,0)
C
...
C       WATERCOST(9,NF): FIXED PRICE PER VOLUME OF NON-ROUTED SURFACE-WATER [$/L3]
        IF(ICOST.EQ.1)
     1  CALL FMP2DPLSTRD(WATERCOST,9,NFARMS,IN,IOUT,2,9)
      ENDIF
C
...
C9A-----READ SEMI-ROUTED DELIVERY PER FARM
      IF(ISRDFL.EQ.1) THEN
      CALL FMP2INTLSTRD(ISRD,5,NFARMS,IN,IOUT)
      WRITE(IOUT,110)
  110 FORMAT(/,1X,
...
C9B-----READ SEMI-ROUTED RUNOFF-RETURNFLOW PER FARM
      IF(ISRRFL.EQ.1) THEN
      CALL FMP2INTLSTRD(ISRR,5,NFARMS,IN,IOUT)
      WRITE(IOUT,112)
  112 FORMAT(/,1X,
...
C       (IEBFL=1: Conservative Behavior - Constant Efficiency over Time Step)
      IF(IEFFL.EQ.1.AND.IEBFL.LE.1) THEN
        DO NF=1,NFARMS
        DO NC=2,1+NCROPS
           DO IR=1,NROW
           DO IC=1,NCOL
              IF(NC.GT.2.AND.EFF(NC,NF).EQ.0D0) THEN
                ...
                EF2D(IC,IR)=EFF(2,NF)
c               EF2D(IC,IR)=(DNINT(EFF(2,NF)*1D+10))/1D+10
                ENDIF
              ELSE
                IF(IFID(IC,IR).EQ.NF.AND.ICID(IC,IR).EQ.NC-1) THEN
                EF2D(IC,IR)=EFF(NC,NF)
c               EF2D(IC,IR)=(DNINT(EFF(NC,NF)*1D+10))/1D+10
                ENDIF
              ENDIF
           ENDDO
           ENDDO
...
C
C     
      SUBROUTINE FMP2RP(IN,ISTARTFL,KPER,IUNITSFR,IUNITMNW,IGRID)
C-----VERSION 2 09/18/2009 FMP2RP
C     ******************************************************************
...
C     -------------------------------------------------------------------
     ...
     2  ICUFL,IPFL,IFTEFL,IIESWFL,IEFFL,IEBFL,IFID,EFF,EF2D,CU,ICID,
     3  CU2D,ETR,EGW,NCROPS,PFLX,PFLR,TPPOT,EPPOT,FIESW,FTE,CROPBEN,
     4  WATERCOST,NRD,UNRD,RNRD,MXNRDT,ISRD,ISRR,ALLOT,TGW,FCSEGL,
     5  FDSEGL,IFCRID, IFDRID,NFSEG,SFRADD,DIVADD,DIVTMP,IDEFFL,IBEN,
     6  ICOST,INRDFL,ISRDFL,ISRRFL,IRDFL,IRRFL,IALLOT,IROTFL,ICCFL,WRC,
     7  GSURF,IRTPFL
c      USE FMPMODULE, ONLY: ETRR,PFLXR
      USE GLOBAL, ONLY:IOUT,NCOL,NROW,NLAY,DELR,DELC,PERLEN,NPER
      USE GWFSFRMODULE, ONLY:ISTRM,STRM,NSTRM,IDIVAR,NSEGDIM,SEG
      IMPLICIT NONE
C     -------------------------------------------------------------------
C        ARGUMENTS:
C     -------------------------------------------------------------------
      INTEGER IGRID,ISTARTFL,IUNITSFR,IN,IUNITMNW,KPER
C     -------------------------------------------------------------------
C        LOCAL VARIABLES:
      ...
      INTEGER N,ITMP,NP,IOUTU,MXACTFW,NREAD,NF,NWPF,NC,IR,IC,L,IT,IRT,  !FORMERLY IMPLICIT INTEGER
     1 IOUF,LOLD1,LOLD2,IFCRIDOLD,NRCH,ICNT,NR,IA,IB
...
c       (info: if consumptive use changes at each time-step, then istartfl is set zero in FMP2AD.)
      ISTARTFL=0
c
C1B-----FOR USE OF SFR1 AND (SEMI-)ROUTED DELIVERIES, SET FLAG TO SKIP FMP2FM IN 1ST ITERATION
C       
...
C1B1----FOR USE OF ACREAGE-OPTIMIZATION WITH "WATER-CONSERVATION POOL" ACCOUNTING ...
      if(IDEFFL.EQ.2) then
          DO N=1,NSEGDIM                                          
C
C     ... SET FLAG SUCH THAT FMP2FM BE SKIPPED IN 1ST ITERATION (MF2K5 WITH SFR7 ONLY):
...
c         If new rate (divadd) is higher than old one (seg(2,n)), then set istartfl=-1 (istartfl=0 in FMP2FM),
c         * to cause istartfl=0 in FMP2FM, and to skip 1st FMP2FM iteration:
...
c         * to cause istartfl=1 during the 2nd FMP2FM iteration, in order to reset
...
c               seg(2,n) is stored as temporay parameter divtmp(n) in FMP2FM and compared with the new rate.      
...
C       skipping FMP2FM in 1st iteration is strictly not necessary if diversion rate doesn't change
...
C       AND NUMBER OF PARAMETERS (NOT YET USED!)
      IF(NPFWL.GT.0) THEN
         READ(IN,*) ITMP,NP
      ELSE
         NP=0
...
C       READ LIST OF ATTRIBUTES ASSOCIATED WITH NON-PARAMETER-FARM-WELLS.
   ...
   99       FORMAT(1X,/1X,'THE NUMBER OF ACTIVE FARM WELLS (',I6,
     1                     ') IS GREATER THAN MXACTFW(',I6,')')
            CALL USTOP(' ')
         END IF
         CALL FMP2WELRD(NNPFWL,FWELL,1,NFWLVL,MXFWEL,IFWLAL+1,IN,IOUT,
     1'WELL NO. LAYER    ROW    COL  F-WELL ID    FARM ID         QMAX',
     2        FWLAUX,5,NAUX,NCOL,NROW,NLAY,6,6,IPFWEL,IUNITMNW)
      END IF
      NFWELS=NNPFWL
C
C2A4----IF THERE ARE ACTIVE FARM WELL PARAMETERS, READ THEM AND SUBSTITUTE
      ...
      IF(NP.GT.0) THEN
         DO 30 N=1,NP
         CALL FMP2WELPARRD(IN,'FMP',IOUTU,'QMAX',FWELL,NFWLVL,MXFWEL,
     1            NREAD,MXACTFW,NFWELS,6,6,
     2'WELL NO. LAYER    ROW    COL  F-WELL ID    FARM ID         QMAX',
     3            FWLAUX,5,NAUX)
   30    CONTINUE
      END IF
C
...
C       SET A ZERO LAYER FOR A MULTIAQUIFER MNW-WELL TO LAYER ONE (TO AVOID ARRAY ERROR IN IBOUND)
      ...
      NWPF=0
         DO L=1,NFWELS
         IF(IUNITMNW.GT.0.AND.IDINT(FWELL(4,L)).LT.0.AND.IALLOT.GT.1)
     1   THEN
  ...
  990         FORMAT(1X,/1X,'THE LINK OF WELL IN ROW ',I6,' AND ',
     ...
     2        ' CANNOT BE COMBINED WITH IALLOT>1')
              CALL USTOP(' ')
         ENDIF
         IF(IUNITMNW.GT.0.AND.IDINT(FWELL(1,L)).EQ.0.AND.
     1      IDINT(FWELL(4,L)).GE.0) THEN
              WRITE(IOUT,991) IDINT(FWELL(2,L)),IDINT(FWELL(3,L))
  991         FORMAT(1X,/1X,'THE MULTI-AQUIFER WELL IN ROW ',I6,' AND ',
     1        'COLUMN ',I6,' MUST HAVE A NEGATIVE FARM-WELL ID')
              CALL USTOP(' ')
         ENDIF
         DO N=1,NAUX
           IF(IDINT(FWELL(6+N,L)).EQ.1) THEN
...
C       (IEBFL=2: cell-by-cell efficiency may vary over time step)
      IF(IEFFL.EQ.2.OR.(IEBFL.EQ.0.OR.IEBFL.EQ.2).OR.IROTFL.EQ.-1) THEN !NOT NECESSARY FOR (IEFFL=1 AND IEBFL=1) --> ALREADY IN RQ
      DO NF=1,NFARMS                                                    !FOR IEFFL=2 AND IEBFL=0 OR 2 NECESSARY IN ORDER TO RESET EFF. FROM
C                                                                       !CALC. EFF. TO SPECIFIED EFFICIENCIES AT BEGINNING OF STRESS PERIOD!
      DO NC=2,1+NCROPS                                                  !NOT FOR NCROPS+3 AND NCROPS+4 (NONZERO CALCULATED VALUES)
         ...
         DO IC=1,NCOL                                                   !VARYING (ROTATING) CROP 2D-ARRAY.
            IF(NC.GT.2.AND.EFF(NC,NF).EQ.0D0) THEN
              IF(IFID(IC,IR).EQ.NF) THEN
              EF2D(IC,IR)=EFF(2,NF)
              ENDIF
...
C4B-----READ DEPTH OF ROOTZONE FOR EACH STRESS PERIOD & CALCULATE 2D-ARRAY OF ROOTZONE-DEPTHS
      IF(IRTFL.EQ.2) CALL FMP2LSTRD(ROOT,2,NCROPS,IN,IOUT,2,2)
      IF(IRTFL.EQ.2.OR.(IRTFL.EQ.1.AND.IROTFL.EQ.-1)) THEN
      DO NC=1,NCROPS
         DO IR=1,NROW
         DO IC=1,NCOL
            IF(ICID(IC,IR).EQ.NC) THEN
...
C             and net-recharge will be calculated for such cells.
      IF(ICUFL.LT.3) THEN
      CALL FMP2LSTRD(CU,3,NCROPS,IN,IOUT,2,2)                           !IF ETC OR KC VALUES ARE TO BE READ
      ENDIF
      IF(ICUFL.LT.3.AND.ICUFL.GT.0) THEN                                !IF ETC VALUES ARE TO BE READ
      DO NC=1,NCROPS                                                    !(ICUFL=2: NO FALLOW CELLS IN ARRAY;
...
c      CALL U2DREL(ETRR,ANAME(3),NROW,NCOL,0,IN,IOUT)
c      ETR=DBLE(ETRR)
      DO IR=1,NROW
      DO IC=1,NCOL
...
C      CALL U2DREL(PFLXR,ANAME(1),NROW,NCOL,0,IN,IOUT)      
C      PFLX=DBLE(PFLXR)
      DO IR=1,NROW
      DO IC=1,NCOL
...
C                                        2. FOR SMALL ET PER DAILY TIME UNIT, INTERCEPT HAS TO BE ZERO!!
C       CROPBEN(4,NC): CROP MARKET PRICE [$/MASS]
        IF(IBEN.EQ.2) CALL FMP2DPLSTRD(CROPBEN,4,NCROPS,IN,IOUT,0,0)
C
...
C8C1----READ EQUALLY APPROPRIATED FARM ALLOTMENT HEIGHT
      IF(IRDFL.NE.0.OR.ISRDFL.GT.0) THEN
        IF(IALLOT.EQ.1) THEN
          READ(IN,*) ALLOT
          WRITE(IOUT,114) ALLOT
  114     FORMAT(1X,/'FARM ALLOTMENT HEIGHT [L]: ',F7.3)
C
C8C2----READ PRIOR APPROPRIATION SYSTEM WATER RIGHTS CALLS
        ELSEIF(IALLOT.EQ.2) THEN
          CALL FMP2DPLSTRD(WRC,2,NFARMS,IN,IOUT,0,0)
...
C
      IF(IRTPFL.GT.0) THEN
      OPEN(UNIT=1009, FILE='ROUT.OUT', STATUS='UNKNOWN')
        IOUF=1009
      ELSEIF(IRTPFL.LT.0) THEN
        IOUF=IOUT
      ENDIF
...
C
      ...
      DO NF=1,NFARMS
      IF(IRTPFL.NE.0) THEN
      WRITE(IOUF,'(//,"ROUTING INFORMATION FOR FARM: ",I8,", PERIOD ",I4
     1 ,/,A51)') NF, KPER,
     2 "---------------------------------------------------"
      ENDIF
C
...
C       - RITE LIST OF FOUND CANAL REACHES AND FARM CANAL SEGMENT LENGTH TO LIST FILE.
      IF(IRTPFL.NE.0) THEN
      WRITE(IOUF,'(/,1X,"DELIVERIES:",/,2X,"FULLY-ROUTED DELIVERIES:")')
      ENDIF
      FCANALSEGL=0.D0
      IF(IRDFL.NE.0.AND.(ISRDFL.EQ.0.OR.(ISRDFL.NE.0.AND.
     1  ISRD(2,NF).EQ.0.AND.ISRD(3,NF).EQ.0.AND.
     2  ISRD(4,NF).EQ.0.AND.ISRD(5,NF).EQ.0) ) ) THEN
        IF(IRTPFL.NE.0) THEN
        IF(IRDFL.EQ.1) THEN
        WRITE(IOUF,'(3X,"ACTIVATED SEARCH FOR REACHES OF DIVERSION ",
     1  "SEGMENTS (CANALS) THAT ARE WITHIN A FARM.")')
        ELSEIF(IRDFL.EQ.-1)THEN
        WRITE(IOUF,'(3X,"ACTIVATED SEARCH FOR REACHES OF ANY STREAM ",
     ...
     2             ISTRM(2,L).EQ.IR.AND.ISTRM(3,L).EQ.IC ) THEN
              ...
              IF(IRTPFL.NE.0) THEN
              IF(LOLD1.EQ.0) WRITE(IOUF,1161)                           !However note that reuse stress period (-1) in input file won't work. Each stress period's inflow must be specified separately, since if -1 is specified the last stored value (i.e. seg(2,n)) will be re-used.
 ...
 1161         FORMAT(3X,'FULLY ROUTED DELIVERY FROM THE FIRST, MOST ',
     ...
     8        ,'REACH_LENGTH')
              WRITE(IOUF,116)ISTRM(2,L),ISTRM(3,L),ISTRM(4,L),ISTRM(5,L)
     1        ,STRM(1,L) 
  116         FORMAT(3X,I6,I8,I12,I11,3X,G15.8)
      ...
      ELSE
        IF(IRTPFL.NE.0) THEN
        IF(IRDFL.EQ.1) THEN   
        WRITE(IOUF,'(3X,"DEACTIVATED SEARCH FOR REACHES OF DIVERSION ",
     1  "SEGMENTS (CANALS) THAT ARE WITHIN A FARM.")')
        ELSEIF(IRDFL.EQ.-1) THEN
        WRITE(IOUF,'(3X,"DEACTIVATED SEARCH FOR REACHES OF ANY STREAM ",
     1  "SEGMENTS THAT ARE WITHIN A FARM.")')
        ELSEIF(IRDFL.EQ.0) THEN
        WRITE(IOUF,'(3X,"ROUTED DELIVERY OPTION WAS NOT SELECTED ")')
        ENDIF
        ENDIF
      ENDIF
...
C       - LOCK DIVERSION INTO CANAL SEGMENT OF "REMOTE" FARM DIVERSION REACH, SO THAT IT CAN'T BE OVERWRITTEN.
      IF(IRTPFL.NE.0) THEN
      WRITE(IOUF,'(/,2X,"SEMI-ROUTED DELIVERIES:")')   
      ENDIF
     ...
     7       ISRD(4,NF).EQ.ISTRM(4,L).AND.ISRD(5,NF).EQ.ISTRM(5,L)))THEN
            ...
            IF(IRTPFL.NE.0) THEN
            WRITE(IOUF,117) ISTRM(2,L),ISTRM(3,L),ISTRM(4,L),ISTRM(5,L)
  117       FORMAT(3X,
...
C8D3----DEFINE NUMBER OF SEGMENT FOR EACH FARM, ONCE AN FARM HEADGATE REACH WITHIN (AUTOMATIC) OR REMOTE FROM (SPECIFIED) A FARM IS FOUND
     ...
     2     IFCRID(NF,L-2).AND.IFCRID(NF,L).NE.IFCRIDOLD) THEN
             ...
             IF(IALLOT.GE.2) NFSEG(NF)=ISTRM(4,L)
            ENDIF
        ENDDO
      ENDIF
C
...
C       - WRITE LIST OF FOUND DRAIN REACHES AND FARM DRAIN SEGMENT LENGTH TO LIST FILE.
      IF(IRTPFL.NE.0) THEN
      WRITE(IOUF,'(/," RETURNFLOWS:",/,2X,"FULLY-ROUTED RETURNFLOWS:")')
      ENDIF
      FDRAINSEGL=0.D0
      IF(ISRRFL.EQ.0.OR.(ISRRFL.NE.0.AND.
     1  ISRR(2,NF).EQ.0.AND.ISRR(3,NF).EQ.0.AND.
     2  ISRR(4,NF).EQ.0.AND.ISRR(5,NF).EQ.0) ) THEN
        IF(IRTPFL.NE.0) THEN
        IF(IRRFL.EQ.1) THEN
        WRITE(IOUF,'(3X,"ACTIVATED SEARCH FOR REACHES OF NON-DIVERSION",
     1  " SEGMENTS THAT ARE WITHIN A FARM.")')
     ...
     2             ISTRM(2,L).EQ.IR.AND.ISTRM(3,L).EQ.IC ) THEN
              ...
              SFRADD(L)=DBLE(STRM(12,L))                                !-------> !RENAME, SO THAT FM AT SECOND AND FOLLOWING ITERATIONS DOESN'T START UP WITH STRM(12,L)=STRM(12,L)+RDR(L), BUT WITH STRM(12,L)=RUNOFF(L)+RDR(L)
              IF(IRTPFL.NE.0) THEN
              IF(LOLD2.EQ.0) WRITE(IOUF,220)                            !However note that reuse stress period (-1) in input file won't work. Each stress period's inflow must be specified separately, since if -1 is specified the last stored value (i.e. seg(2,n)) will be re-used.
  220         FORMAT(3X,'FULLY ROUTED RUNOFF-RETURNFLOW PRORATED ',
     1        'OVER REACHES WITHIN THE FARM AT:
     2        ',/6X,'ROW',2X,'COLUMN',2X,'SEGMENT_NO.',2X,'REACH_NO.',2X
     3        ,'REACH_LENGTH')
              WRITE(IOUF,221)ISTRM(2,L),ISTRM(3,L),ISTRM(4,L),ISTRM(5,L)
     1        ,STRM(1,L)
  221         FORMAT(3X,I6,I8,I12,I11,3X,G15.8)
      ...
      ELSE
        IF(IRTPFL.NE.0) THEN
        IF(IRRFL.EQ.1) THEN
        WRITE(IOUF,'(3X,"DEACTIVATED SEARCH FOR REACHES OF ",
...
C       (RECHARGE CUMULATIVE RUNOFF INTO THAT DRAIN REACH IN FMP2FM-C10B1)
C       (FMP2RP-C8D5 WAS FORMERLY (PRIOR TO 07/02/08) LOCATED IN FMP2FM-C10B2)
      IF(IRTPFL.NE.0) THEN
      WRITE(IOUF,'(/,2X,"SEMI-ROUTED RETURNFLOWS:")')
      ENDIF
      NRCH=0
      IF(FDSEGL(NF).EQ.0 .AND.(ISRRFL.EQ.0.OR.(ISRRFL.NE.0.AND.
     1  ISRR(2,NF).EQ.0.AND.ISRR(3,NF).EQ.0.AND.
     2  ISRR(4,NF).EQ.0.AND.ISRR(5,NF).EQ.0)))THEN 
        ICNT=0
        GSURFOLD=0.D0
        ...
        DO IR=1,NROW
        DO IC=1,NCOL
        IF(FDSEGL(NF).EQ.0.AND.IFID(IC,IR).EQ.NF) THEN   
         ICNT=ICNT+1
...
C       - LOCK RUNOFF INTO "REMOTE" FARM RETURNFLOW REACH, SO THAT IT CAN'T BE OVERWRITTEN.
     ...
     7       ISRR(4,NF).EQ.ISTRM(4,L).AND.ISRR(5,NF).EQ.ISTRM(5,L)))THEN
            ...
            SFRADD(L)=DBLE(STRM(12,L))
            IF(IRTPFL.NE.0) THEN
            WRITE(IOUF,119) ISTRM(2,L),ISTRM(3,L),ISTRM(4,L),ISTRM(5,L)
  119       FORMAT(3X,'SEMI-ROUTED RUNOFF-RETURNFLOW TO A SPECIFED ',
     ...
     2  ," NO SEMI-ROUTED RETURNFLOW POSSIBLE.")')
      ...
      ENDIF
C
      ENDDO                                                             !END OF FARMS DO-LOOP
      IF(IRTPFL.NE.0) WRITE(IOUF,'(//)')
C
...
C              net-recharge will not be calculated for such cells.
      IF(ICUFL.LE.2 .AND. IPFL.EQ.2) THEN
      DO IR=1,NROW
      DO IC=1,NCOL
...
C
C
      SUBROUTINE FMP2AD(ISTARTFL,KPER,IGRID)
C-----VERSION 2 09/18/2009 FMP2AD
C*********************************************************************
...
c       (Info: if consumptive use changes only at each stress period, then istartfl is set zero only in FMP2RP.)
...
c         even if link to sfr does not exist, istartfl=-1 causes istartfl=1 during the 2nd FMP2FM iteration,
...
C       AND CREATE 2D-ARRAY OF ROOT-ZONE DEPTHS FOR ALL CELLS
      ...
      RTDSUM=0.D0
      DO L=ISTART,ISTOP
      RTDSUM=RTDSUM+RTD(NC,L)
      ENDDO
      ROOT(2,NC)=RTDSUM/DBLE(ISTOP-ISTART+1)
      ENDDO
      DO NC=1,NCROPS
         DO IR=1,NROW
         DO IC=1,NCOL
...
C
C
      SUBROUTINE FMP2FM(KITER,KPER,KSTP,ISTARTFL,IUNITSFR,IUNITMNW,
     1                  IUNITUZF,IGRID)
C-----VERSION 2 09/21/09 FMP2FM
C     ******************************************************************
...
C     ------------------------------------------------------------------
     ...
     2    MXFWEL,TDR,IFID,ICID,TFDR,TFDROLD,FWELL,QMAXF,QREQ,QREQOLD,
     3    QAVF,QDEF,QEXC,NFARMS,NCROPS,NFWLVL,NWPERF,NRD,UNRD,RNRD,
     4    MXNRDT,IFCRID,IFDRID,NFSEG,RDR,FLOWINMIN,QSTRMIN,FCSEGL,
     ...
     8    RISERUN,IALLOT,ALLOT,KNTR,IIESWFL,IRDRFL,PCLOSE,IEBFL,ISDPFL,
     9    IOPFL,WRC,IPAPFL,TTOT,ETOT,DPERC,EGWA,IWELLFLD
      USE FMPBLK
      USE GLOBAL,       ONLY:IOUT,RHS,HCOF,IBOUND,HNEW,NCOL,NROW,NLAY,
     1                       DELR,DELC,LENUNI,PERLEN,NPER
      USE GWFSFRMODULE, ONLY:STRM,ISTRM,NSTRM,SEG,DVRSFLW,NSEGDIM,
     1                       IDIVAR,SGOTFLW
      USE GWFMNWMODULE, ONLY:WELL2,NWELL2
      USE GWFUZFMODULE, ONLY:IUZFBND,FINF,EXCESPP,VKS
      IMPLICIT NONE
C     ------------------------------------------------------------------
C        ARGUMENTS:
C     ------------------------------------------------------------------
      INTEGER KITER,KPER,KSTP,ISTARTFL,IUNITSFR,IUNITMNW,IUNITUZF,IGRID
C     ------------------------------------------------------------------
C        FUNCTIONS AND COMMON BLOCKS:  
      EXTERNAL RTFUNC, FUNC 
      COMMON ITER, ITER2
C     ------------------------------------------------------------------ 
C        LOCAL VARIABLES:
      INTEGER NF,L,ITER2,ITER,NC,IR,IC,IOPTFL,N,NOD,IL,ISNGLWELL,M,J,I, !FORMERLY IMPLICIT INTEGER
     1 ID,NT,IDDELF,NSEGOLD,IT,ITTOT,IRT,ITU,LL,NR,IA,IB,K,NCELL,NCON,
     ...
     7  SWRUNSUM,AVGSUPPLYFLR,AVGSUPPLYFLUX,QSUMDF,QSUMEX,QDF,QEX,
     8  THISAT1,THDSAT1,THISAT2,THDSAT2,TGWSAT1,TGWSAT2,ALPHA,RTFUNC,
     9  FUNC,QACT,QMAXMNW2,QMAXMNW3,FIESWI,FIESWP,TG,DRDC,
     ...
     +  ,SUPPLYFLRNEWOLD,SWPREV,SWOLDPREV,SWREDPREV     
C     ------------------------------------------------------------------
      CALL SFMP2PNT(IGRID)
C
...
c
c          ISTARFL=-1 is advanced to ISTARTFL=0 to skip 1st FMP2FM iteration (see comment C4 below) 
...
c
c       2. ISTARTFL=0 INDICATES FIRST FMP2FM ITERATION:
C 
C          If ISTARTFL=0 is advanced to ISTARTFL=1 then the 2nd MF2K5, but first FMP2FM iteration occurs.
c          ISTARTFL=0 is advanced to ISTARTFL=1 and the 2nd MF2K5, but first FMP2FM iteration.
c          Hence, ISTARTFL=0 tells when to reset some parameters at the beginning of the first FMP2FM
...
c        iunitsfr.gt.0; even if IRDFL.eq.0!                             !therefore: drain runoff is cumulative! However, don't carry over runoff from previous iteration.
      ...
      enddo                                                             !strm(12,l)=sfradd(l)-RDEL 
C
C3===== FOR PRIOR APPROPRIATION: RESET IRDRFL(NF), RDR(L), AND SEG(2,NSEG) ==================================
C
      if(iallot.GE.2) then                                              !IRDRFL(NF)=0: tells to allow PRIOR for farm NF; IRDRFL(NF)=1 tells that PRIOR was solved for a farm NF.
         if((IRDFL.NE.0.OR.ISRDFL.GT.0).and.istartfl.eq.0) then         !- Set IRDRFL=0 only AT BEGINNING of new timestep to allow PRIOR for all farms within the time step!
            DO NF=1,NFARMS                                              !- Do not reset IRDRFL to zero for all farms, if ISTARTFL > 0 indicating that the process of
               IRDRFL(NF)=0                                             !  "cumulating PDIV, exiting PRIOR & FMP2FM, solving MF2K5, and restarting FMP2FM" is still ongoing
               QSTRMIN(NF)=0.D0
            ENDDO                                                       !  during the present time step! 
C                                                                       !  (After cumulating PDIV, routines PRIOR and FMP2FM are exited if there was no solution achieved. 
            do l=1,nstrm                                                !   Before solving the gwf-equation in MF2K5, ISTARTFL is set zero, and ISTARTFL=1 when FMP2FM restarts).
               rdr(l)=0.D0                                             
...
C
C4A-----RESET EFFICIENCY FROM CALCULATED EFFICIENCY TO SPECIFIED EFFICIENCIES AT BEGINNING OF EACH ITERATION
...
C4B-----INITIALIZE SOME VARIABLES 
     ...
     1   AND.ISTARTFL.EQ.0).OR.
     2   (IUNITSFR.EQ.0.OR.(IRDFL.EQ.0.AND.ISRDFL.EQ.0).
     3   AND.ISTARTFL.EQ.1)) THEN     
        DO NF=1,NFARMS      
        TFDR(NF)=0.D0
        TFDROLD(NF)=0.D0
        NRD(1,NF)=0.D0
      ...
      ENDDO
C
C4C-----SKIP 1ST FMP2FM-ITERATION
      if(istartfl.le.0) GO TO 300
C
C4D-----INITIALIZE SOME LOCAL VARIABLES
      FARMALLOT=0.D0
C
C5===== UPDATE QMAXF FOR MNW IF QMAX OF MNW-WELLS ITERATIVELY CHANGES =======================================
C       (ONLY IN FM IF FMP IS LINKED TO MNW AND QMAX IS ITERATIVELY UPDATED. -- QMAX NORMALLY DONE IN RP)
      IF(IUNITMNW.GT.0) THEN
      DO N=1,NAUX
      DO NF=1,NFARMS
       IF((ISTARTFL.EQ.1.OR.KITER.EQ.1).AND.
     1  FWLAUX(N).EQ."QMAXRESET") THEN
        QSUMF=0.D0
          DO L=1,NFWELS
            IF(IDINT(FWELL(5,L)).EQ.NF) THEN
              IF(FWELL(6+N,L).NE.0.D0) THEN
              FWELL(6,L)=FWELL(6+N,L)
              ENDIF
            QSUMF=QSUMF+FWELL(6,L)
            ENDIF
          ENDDO
        QMAXF(NF)=QSUMF
        IF(NF.EQ.NFARMS) GOTO 1
        ENDIF
      ENDDO
      ENDDO
      DO NF=1,NFARMS
        IF(KITER.GT.2) THEN                                             !WELL2(3,N)=qact is not calculated in K=1. I.e., comparison between Well2(2 and Well2(3 is not yet possible in next iteration K=2 (FMP comes first). So first comparison is possible for K>2
          QSUMF=0.D0
          DO L=1,NFWELS
          IF(IDINT(FWELL(5,L)).EQ.NF) THEN
...
C
              ...
              DO M=1,NLAY
                IF(IDINT(WELL2(1,N))-(M-1)*NROW*NCOL.EQ.NOD) THEN
                  QMAXMNW2=QMAXMNW2+WELL2(2,N)
                  QMAXMNW3=QMAXMNW3+WELL2(3,N)
                ENDIF
              ENDDO
              ENDDO
...
C6A2----FOR ALL ACTIVE CROP CELLS
      IF(ICID(IC,IR).GT.0) THEN
C
C6A3----IF CELL IS CONSTANT HEAD MOVE ON TO NEXT HORIZONTAL LOCATION (DELETED IN FMP2).
...
C        to RUNOFF to avoid losing mass).
      IF(IBOUND(IC,IR,IL).EQ.0.AND.IL.NE.NLAY) THEN
      TDR(IC,IR)=0.D0
      GO TO 60
      ENDIF
C 
...
C       (UPTAKE FROM SATURATED ROOT ZONE IN C9B1D --> IRRELEVANT FOR CALCULATION OF IRRIGATION REQUIREMENT)
      ...
      ELSE
         PSI1=0.D0
      ENDIF
      IF(PSI(4,ICID(IC,IR)).LT.0.D0) PSI2=DABS(PSI(4,ICID(IC,IR)))
      IF(PSI(5,ICID(IC,IR)).LT.0.D0) PSI3=DABS(PSI(5,ICID(IC,IR)))
C
...
C                                         FOR DEPTH(PSIWET) AND FOR DEPTH(PSIDRY)
      XWET=RTFUNC(func,DRZ,PSI3*MLT,NEXP,PSIWET*MLT,EPS,j,ic,ir)/MLT
      ENDIF
      XDRY=RTFUNC(func,DRZ,PSI3*MLT,NEXP,PSIDRY*MLT,EPS,j,ic,ir)/MLT
...
C
      DO N=1,NAUX
        IF(FWLAUX(N).EQ."NOCIRNOQ") THEN
        DO NF=1,NFARMS
          IF(ISTARTFL.EQ.1.OR.KITER.EQ.1) THEN
          QSUMF=0.D0
            DO L=1,NFWELS
              IF(IDINT(FWELL(5,L)).EQ.NF) THEN
                IF(FWELL(6+N,L).NE.0.D0) THEN
        ...
        DO NF=1,NFARMS
          ...
          ID=IDINT(FWELL(5,L))
          IF(ID.EQ.NF)THEN
            IF(FWELL(6+N,L).NE.0.D0.AND.TDR(IC,IR).LT.FPS) THEN
            FWELL(6,L)=0.D0
            ENDIF
            QSUMF=QSUMF+FWELL(6,L)
          ENDIF
...
C6G1----CALCULATE TOTAL FARM DELIVERY REQUIREMENT FOR EACH FARM
      ...
      TFDR(NF)=0.0D0
        DO IR=1,NROW
        DO IC=1,NCOL
           IF(IFID(IC,IR).EQ.NF.AND.ICID(IC,IR).GT.0) THEN
             TFDR(NF)=TFDR(NF)+TDR(IC,IR)
           ENDIF      
        ENDDO
        ENDDO
      ENDDO         
...
C       pre-deficiency budget was corrected and saved (and iter2 was set 1)
      IF(IALLOT.GE.2.and.(ideffl.eq.-2.or.ideffl.gt.0)
...
C        and groundwater pumping (decreasing priority), which are to be determined in C7E)
      ...
      IOPTFL=0
        ...
        DO IC=1,NCOL
C
C7B1----SET ACREAGE OF ALL CELLS IN DOMAIN TO -100% (FAKE PERCENTAGE, THAT IS ONLY USED
...
C       then --> exit --> new iteration is not a new beginning
               if(iallot.GE.2.and.iter2.eq.1.and.iter.eq.1) goto 402
C
...
C              for iallot>=2, all farms are run prior to acreage optimization)
               REDPCT(IC,IR)=1.D0
C
...
C       save head-dep. efficiency from last iteration --> for closure criterion in FMP2WELBD
...
C      (ALLOMENT-DEPTH x FARM AREA)/LENGTH OF STRESS PERIOD
      IF(IALLOT.EQ.1.AND.IUNITSFR.GT.0) THEN  
      AREASUM=0.D0
        DO IR=1,NROW
        DO IC=1,NCOL
...
C7E2----UPDATE SW DELIVERY FROM HEAD-GATE REACH (ROUTED OR SEMI-ROUTED), AND FARM WELL DISCHARGE
      IF(IUNITSFR.GT.0.AND.(IRDFL.NE.0.OR.ISRDFL.GT.0)) THEN
         ...
         DO L=3,NSTRM
            LL=L-1                              
            FLOWIN=DBLE(ANINT(STRM(9,LL)*AR))/AC                        !STRM(10,L) ?
...
C             GENERAL CASE & EQUAL APPROPRIATION
              IF(IALLOT.EQ.1.AND.FARMALLOT.LT.FLOWIN) FLOWIN=FARMALLOT
              IF(IALLOT.LT.2.AND.RDEL.GT.FLOWIN)      RDEL=FLOWIN
C
C             PRIOR APPROPRIATION
              IF(IALLOT.GE.2) THEN
C
C               check if delivery is constrained by water rights calls
                IF(IALLOT.EQ.2) RDEL=MIN(RDEL,WRC(2,NF))
C
...
C               carry out prior appropriation subroutine
                IF(IRDRFL(ifcrid(nf,L)).EQ.0) THEN
                  CALL PRIOR(RDEL,FLOWIN,L,NF,ISTARTFL,KPER,KSTP)
                  IF(ITER.EQ.1) THEN
                     IF(ideffl.eq.-2.or.ideffl.gt.0) THEN
...
c       usually are only printed from within the deficiency scenario routine below (cmpr. format 11 and 12).
      IF(iallot.GE.2.and.(ideffl.eq.-2.or.ideffl.gt.0).and.
...
C
C7E5A---SAVE BUDGET TERMS IN GENERAL IN ODER TO BE AVAILABLE IN FMP2WELBD
...
C       under a different name in order to be still available in FMP2WELBD along with the final terms.
      TFDR(NF)=TF
      NRD(1,NF)=ND
      QREQ(NF)=QR
C
...
C       the farms loop while deficieny scenarios are not yet executed.
      if(IALLOT.GE.2.and.iter2.eq.0) then
      TFDROLD(NF)=TF
      NRD(2,NF)=ND
      QREQOLD(NF)=QR
      go to 80
...
C           
            ...
            DO IC=1,NCOL
               IF(IFID(IC,IR).EQ.NF) THEN
                 DO NC=1,NCROPS
                    IF(ICID(IC,IR).EQ.NC.AND.IFALLOW(2,NC).EQ.0)THEN    !for all non-fallowable crops
                    AREASUM=AREASUM+DBLE(DELR(IC)*DELC(IR))
                    ENDIF
                 ENDDO
               ENDIF
...
C                   set supply zero for all fallowable crops
                    ELSEIF(ICID(IC,IR).EQ.NC.AND.IFALLOW(2,NC).NE.0)THEN
...
C
C                  GENERAL CASE & EQUAL APPROPRIATION
                   IF(IALLOT.EQ.1 .AND. FARMALLOT.LT.FLOWIN)
     1             FLOWIN=FARMALLOT                                     !Available Flow = Allotment
                   IF(IALLOT.LT.2 .AND.RDELCOR.GT.FLOWIN) RDELCOR=FLOWIN
C
C                  PRIOR APROPRIATION
                   IF(IALLOT.GE.2) THEN
C
C                    check if delivery is constrained by water rights calls
                     IF(IALLOT.EQ.2) RDELCOR=MIN(RDELCOR,WRC(2,NF))
C
C                    tell to carry out PRIOR by setting IRDRFL=0 for the following case:
...
C                    carry out prior appropriation subroutine
                     IF(IRDRFL(ifcrid(nf,L)).EQ.0) THEN
                     CALL PRIOR(RDELCOR,FLOWIN,L,NF,ISTARTFL,KPER,KSTP)
                       IF(ITER.EQ.1) then
                         iter2=2
                         GO TO 300
                       endif
...
C
   30 IF(IUNITSFR.GT.0.AND.(IRDFL.NE.0.OR.ISRDFL.GT.0).AND.
     1 (QTOL.GT.FPS.OR.(IDEFFL.GE.3.AND.TF.GT.0D0)) )THEN
      ...
      DO IR=1,NROW
      DO IC=1,NCOL
        IF(IFID(IC,IR).EQ.NF) THEN                                      !DON'T CALCULATE ANYMORE IF DNR IS ALREADY CALCULATED (DNR > 0)!
...
C
      ...
      DO IR=1,NROW
      DO IC=1,NCOL
        IF(IFID(IC,IR).EQ.NF .AND. DWE(IC,IR).EQ.0.D0) THEN
        DWSUM=0.D0
        DO L=1,NFWELS
        QMAX=FWELL(6,L)
          IF(IDINT(FWELL(5,L)).EQ.NF) THEN
          ...
          NC=IABS(IDINT(FWELL(3,L))-IC)                                 !NUMBER OF COLUMNS BETWEEN WELL AND CELL
            ...
            IF(NR.GT.1) THEN
            DO N=2,NR
              IF(IDINT(FWELL(2,L)).LT.IR) IA=IDINT(FWELL(2,L))+(N-1)    !ROW NUMBER, WHOSE DELC WILL BE ADDED UP, STARTING FROM WELL!
              IF(IDINT(FWELL(2,L)).GT.IR) IA=IR+(N-1)                   !ROW NUMBER, WHOSE DELC WILL BE ADDED UP, STARTING FROM CELL!
              ASUM=ASUM+DBLE(DELC(IA))
            ENDDO
            ENDIF
            IF(NC.EQ.0) BSUM=0.D0
...
C         (AV. LIFT AND AV. TOP-ELEVATION OF ALL WELLS IN A FARM WEIGHTED BY THEIR QMAX):
      ...
      IF(IDINT(FWELL(5,M)).EQ.NF) THEN
         IF(IUNITMNW.GT.0.AND.IL.EQ.0) THEN
           DO N=1,NWELL2
             if(well2(8,n).lt.1D30.and.well2(7,n).lt.1D30) then
               if(n.eq.1)then
                 k=IDINT(well2(1,n))
           ...
           ENDDO
         ENDIF           
         IF(IL.GT.0) HL=GSURF(IC,IR)-HNEW(IC,IR,IL)
         HSUM=HSUM+QMAX*HL
...
C
C     NO MORE USE OF EFFICIENCY IN OBJECTIVE FUNCTION IN FMP2:
...
C     THE COST OF DELIVERY IS ASSOCIATED WITH TOTAL DELIVERY (FMP2) AND
C       NOT JUST THE EFFICIENTLY USED CIR (FMP1), AND
C     RESOURCE CONSTRAINTS ARE RELATED TO THE TOTAL AVAILABLE RESOURCE (FMP2) AND
...
C
      ...
      DO NC=1,NCROPS     
         DO IR=1,NROW
         DO IC=1,NCOL 
            IF (IFID(IC,IR).EQ.NF .AND. ICID(IC,IR).EQ.NC) THEN
...
4031    IF(ISDPFL.EQ.-3) THEN
      WRITE(IOUT,13)'CORRECTION FOR TFDR, NR-SWD, R-SWD, & QREQ IF TOTAL
     1AVAILABLE SUPPLY > DEMAND OF PROFITABLE ACREAGE:'
   13 FORMAT(1X,/,A101)
        ENDIF
C
      ELSE   
C
        IF(ISDPFL.EQ.-3) THEN
      WRITE(IOUT,14)'CORRECTION FOR TFDR,NR-SWD,R-SWD,QREQ IF DEMAND OF 
...
C
      ...
      QREQ(NF)=QR
C
      ENDIF                                                             ! END OF CONDITION: if (iallot.LT.2)
C
...
C9A3----IF CELL IS INACTIVE MOVE DOWN A LAYER.
      IF(IBOUND(IC,IR,IL).EQ.0.AND.IL.NE.NLAY) GO TO 160
C
...
c     no percolation is possible. Assumption: all inefficient losses go to surface water
     ...
     1   (IBOUND(IC,IR,IL).EQ.0.AND.IL.EQ.NLAY) ) THEN
      FIESWP=1.D0
      FIESWI=1.D0
      ENDIF
C
...
C                            PSI(DEPTH)   IS SOLVED ITERATIVELY BY BISECTION-METHOD
C                                         FOR DEPTH(PSIWET) AND FOR DEPTH(PSIDRY)
      XWET=RTFUNC(func,DRZ,PSI3*MLT,NEXP,PSIWET*MLT,EPS,j,ic,ir)/MLT    !DRZ ORIGINALLY DERIVED IN CM
      ENDIF
      XDRY=RTFUNC(func,DRZ,PSI3*MLT,NEXP,PSIDRY*MLT,EPS,j,ic,ir)/MLT
      IF(XWET.LT.0D0) XWET=0D0     
...
C       (not for ideffl<0, since tdr may be equal to available deficit irrigation supply [NOT demand])
      IF(IDEFFL.GE.0) TDR(IC,IR)=CIR/EF2D(IC,IR)
C
...
C       (BOTH SPECIFIED AND CLOSEST REACH ARE DEFINED TO BE FARM DRAIN REACHES, I.E.: IFDRID.EQ.NF)
      FDSLEN=FDSEGL(NF)   
        ...
        ENDDO
C
CWSCHMID-07/02/08: CHECK FOR REMOTE REACH OUTSIDE FARM CLOSEST TO LOWEST ELEVATION WAS MOVED TO FMP2RP.
C
...
c            from downstream junior farm and upstream senior farm are 0 and 1005 respectively for optimization!)
              if(ideffl.gt.0.AND.IALLOT.NE.2) then
                n=2
                istartfl=0
...
c           (input terms into scenario do not have to be the corrected ones).
            if(ideffl.eq.-2.AND.IALLOT.NE.2) then  
              n=2
              CALL PRIORCOR(TFDROLD,N,QREQOLD)
            endif
...
C              SHARE WHICH ACCOUNTS FOR THE DEFICIENCIES OF 'WELLS WITH LESS THAN QAVF'
         IF (QMAX.LE.QAVF(ID)) THEN 
         Q=-QMAX
         ELSE
         Q=-(QAVF(ID)+(QDEF(ID)*(QMAX-QAVF(ID))/QEXC(ID)))
         ENDIF
      ENDIF
C
C13C3---FOR WELLS SIMULATED WITH THE MNW-PACKAGE:
C       SEND PUMPING RATE AS "DESIRED PUMPING-RATE" TO MNW-PACKAGE
      IF(IUNITMNW.GT.0.AND.IDINT(FWELL(4,L)).LT.0) THEN
      NOD=(IL-1)*NROW*NCOL + (IR-1)*NCOL + IC
      N=0
      QACT=0.D0
...
C         BETWEEN FMP AND MNW IN A LAST ITERATION
          IF(ILOLD.NE.0) QACT=(WELL2(7,N)-HNEW(IC,IR,IL))*WELL2(11,N)
        ENDIF
        ENDDO
      IF(ILOLD.NE.0.AND.QACT.GT.Q.AND.-Q.EQ.FWELL(6,L).AND.
     1   WELL2(11,N).NE.-1D0) THEN
        FWELL(6,L)=-QACT
        QMAXF(ID)=QMAXF(ID)-(QACT-Q)
        Q=QACT
      ENDIF
        FWELL(NFWLVL,L)=Q
      ELSE
...
C14==== RETURN ==============================================================================================
  300 RETURN
      END
C
      SUBROUTINE FMP2QCNVG(IUNITMNW,IUNITPCG,IUNITSIP,IUNITDE4,
     1                     IUNITGMG)
C-----VERSION 1 09/21/09 FMP2QCNVG
C     ******************************************************************
...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE FMPMODULE,    ONLY:NFWELS,FWELL,QCLOSE,HPCT,RPCT
      USE FMPBLK,       ONLY:FPS
...
C        ARGUMENTS:      
C     ------------------------------------------------------------------
      INTEGER IUNITMNW,IUNITPCG,IUNITSIP,IUNITDE4,IUNITGMG
C     ------------------------------------------------------------------
C        LOCAL VARIABLES:
C     ------------------------------------------------------------------
      INTEGER N,L,NOD,IR,IC,IL,M,J,I                                    !FORMERLY IMPLICIT INTEGER
...
C1======DEFINE HCLOSE AND RCLOSE OF VARIOUS SOLVERS
      ...
      ENDIF      
C
C2======LOOP THROUGH ALL MNW-WELLS LINKED TO FMP
      N=0
        DO WHILE (N.LT.NWELL2)
        N=N+1
          DO L=1,NFWELS
...
C
C      
      SUBROUTINE FMP2WELBD(KSTP,KPER,IUNITMNW,IUNITUZF,IGRID)
C-----VERSION 2 09/18/2009 FMP2WELBD
C     ******************************************************************
...
C     ------------------------------------------------------------------
     ...
     4    TGWA,FCSEGL,EPPOT,TPPOT,IRDFL,ISRDFL,FWLAUX,ICCFL,IFCRID
      USE FMPBLK
      USE GWFMNWMODULE, ONLY:IWL2CB,NWELL2,WELL2
      USE GWFUZFMODULE, ONLY:IUZFBND,IRUNBND,EXCESPP,SEEPOUT,REJ_INF
...
C        ARGUMENTS:
C     ------------------------------------------------------------------
      INTEGER KSTP,KPER,IUNITMNW,IUNITUZF,IGRID
C     ------------------------------------------------------------------
C        LOCAL VARIABLES:
C     ------------------------------------------------------------------
     ...
     2 EP,TP,EI,TI,FDS(9),VAL(24),QMAXTOL,NDOUT,SRDOUT,RDOUT,QFOUT
C     ------------------------------------------------------------------
      CALL SFMP2PNT(IGRID)
C
...
C       (TO BUILD TIME SERIES FOR FARMS: PER,STP,AND TIME ARE INCLUDED FOR EACH FARM)
      ...
      OPEN(UNIT=1001, FILE='FDS.OUT', STATUS='UNKNOWN')
      IF(KPER.EQ.1.AND.KSTP.EQ.1) WRITE(1001,31) TIMEUNIT
   31 FORMAT
     1(2X,'PER',2X,'STP',A14,4X,'FID',9X,'OFE',14X,
...
C       (TO BUILD TIME SERIES FOR FARMS: PER,STP, AND TIME ARE INCLUDED FOR EACH FARM)
      ...
      WRITE(IOUT,1) ISDPFL,KSTP,KPER
    1 FORMAT(1X,'SAVING "FARM DEMAND AND SUPPLY" ON UNIT',I4,
...
C4A1A   HEADER FOR COMPACT FARM BUDGET
      IF(IFBPFL.EQ.1) THEN
      OPEN(UNIT=1008, FILE='FB_COMPACT.OUT', STATUS='UNKNOWN')
      IF(KPER.EQ.1.AND.KSTP.EQ.1) WRITE(1008,41) TIMEUNIT
   41     FORMAT(
     1  '         PER','         STP',A14           ,'         FID',
...
C4A1B   HEADER FOR DETAILED FARM BUDGET
      ...
      OPEN(UNIT=1008, FILE='FB_DETAILS.OUT', STATUS='UNKNOWN')
      IF(KPER.EQ.1.AND.KSTP.EQ.1) WRITE(1008,42) TIMEUNIT
   42     FORMAT(
     1  '         PER','         STP',A14           ,'         FID',
...
C       (TO BUILD TIME SERIES FOR FARMS: PER,STP, AND TIME ARE INCLUDED FOR EACH FARM)
      ...
      FBTYPE="COMPACT "
      IF(INT(IFBPFL/2).EQ.INT((IFBPFL+1)/2)) FBTYPE="DETAILED"
      WRITE(IOUT,43) FBTYPE,IFBPFL,KSTP,KPER
   43 FORMAT(1X,'SAVING "',A8,' FARM BUDGET" ON UNIT',I4,
     1          ' AT TIME STEP',I3,', STRESS PERIOD',I4)
      IF(KPER.EQ.1.AND.KSTP.EQ.1) THEN
C
...
C     
      ...
      ELSEIF(IDEFFL.GT.0) THEN
      write(IOUT,23)NF,EFF(NCROPS+2,NF),TF,ND,TF-ND-QR,QR,QF,DUM1,DUM2,
     1 DUM3
      ENDIF
...
C5D1----PRINT TO ASCII FILE
     ...
     1  (DABS(FDS(I)).LT.1D-1.AND.DABS(FDS(I)).GT.FPS)) THEN
      ...
      fmt_string='(2I5,A14,I7,F12.4,
     1 '//TRIM(ADJUSTL(fmt_number))//'(X,A),I10)'
      WRITE(1001,FMT=TRIM(fmt_string)) KPER,KSTP,TIME,NF,
     1 EFF(NCROPS+2,NF),(FDSVAL(I),I=1,9),IDEFFL
C
C5D2----PRINT TO BINARY FILE
      ELSEIF((ISDPFL.GT.1.AND.IBDOPT.NE.2).OR.
     1       (ISDPFL.GT.1.AND.ICBCFL.EQ.2)) THEN
      write(ISDPFL)
     1 KPER, KSTP, TIME, NF, EFF(NCROPS+2,NF),
...
C6B-----CUMULATE RATES FOR FARMS THAT ARE NOT YET AVAILABLE PER FARM AS GLOBAL VARIABLES
      ...
      DO IR=1,NROW
        ...
        EG=EG+EGWA(IC,IR)
        IF(IUNITUZF.GT.0.AND.ICCFL.EQ.3)THEN
          IF(IUZFBND(IC,IR).GT.0.AND.IRUNBND(IC,IR).GT.0)THEN
            SR=SR+SWRUN(IC,IR)
...
C6G----PRINT FARM BUDGET TO ASCII FILE
     ...
     1  (DABS(VFB(I,NF)).LT.1D-1.AND.DABS(VFB(I,NF)).GT.FPS)) THEN
      ...
      fmt_string=
     1'(2I12,A14,I12,X,'//TRIM(ADJUSTL(fmt_number))//'(A,X))'
      WRITE(1008,FMT=TRIM(fmt_string))
...
C6H-----PRINT FARM BUDGET TO BINARY FILE
     ...
     1       (IFBPFL.GT.2.AND.ICBCFL.EQ.2)) THEN
      ...
      IVAL=12
      ENDIF
      WRITE(IFBPFL) KPER, KSTP, TIME, NF,
...
C
      IF(IOPFL.EQ.-1.OR.IOPFL.EQ.1) THEN
        IF(IOPFL.EQ.-1) IOUTFILE=IOUT
        IF(IOPFL.EQ.1) THEN
          OPEN(UNIT=1006, FILE='ACR_OPT.OUT', STATUS='UNKNOWN')
          IOUTFILE=1006
        ENDIF
        WRITE(IOUTFILE,40) KPER,KSTP,NFARMS,TIME,TIMEUNIT
   40   format(1X,/,1X,
     ...
     3  A14,1X,A14) 
        ...
        fmt_string='(1X,'//TRIM(ADJUSTL(fmt_number))//'F8.3)'
        DO I=1,NROW
        WRITE(IOUTFILE,FMT=TRIM(fmt_string)) (REDPCT(J,I),J=1,NCOL)
        ENDDO
      ENDIF
C     
      ENDIF
...
C       (TO BUILD TIME SERIES FOR FARM WELLS: PER,STP,AND TIME ARE INCLUDED FOR EACH FARM WELL)
      IF(IFWLCB.EQ.1) THEN
      OPEN(UNIT=1002, FILE='FWELLS.OUT', STATUS='UNKNOWN')
      IF(KPER.EQ.1.AND.KSTP.EQ.1) WRITE(1002,33) TIMEUNIT
   33 FORMAT
     1(2X,'PER',2X,'STP',A14,4X,
     2'FARM-WELL ID  FARM ID  LAYER    ROW    COL         RATE')
      ENDIF
C
...
C     il=1
      ...
      QQ=ZERO
C
C8F2----IF THE CELL IS NO-FLOW OR CONSTANT_HEAD, IGNORE IT.
...
C       (MULTI-AQUIFER WELL PUMPAGE INDICATED BY ZERO LAYER NUMBER)
   ...
   62    FORMAT(1X,
     1   'FARM-WELL ID  FARM ID  LAYER    ROW    COL         RATE')
         ENDIF
         IF(IUNITMNW.GT.0.AND.IDINT(FWELL(1,L)).EQ.0) IL=0
         WRITE(IOUT,63) IDINT(FWELL(4,L)),IDINT(FWELL(5,L)),IL,IR,IC,QQ
   63    FORMAT(4X,2I9,3I7,1PG17.8)
         IBDLBL=1
         IF(IUNITMNW.GT.0.AND.IDINT(FWELL(1,L)).EQ.0) THEN
...
C       (MULTI-AQUIFER WELL PUMPAGE INDICATED BY ZERO LAYER NUMBER)
      IF(IFWLCB.EQ.1) THEN
         IF(IUNITMNW.GT.0.AND.IDINT(FWELL(1,L)).EQ.0) IL=0
        WRITE(1002,64)
     1   KPER,KSTP,TIME,IDINT(FWELL(4,L)),IDINT(FWELL(5,L)),IL,IR,IC,QQ
   64    FORMAT(2I5,A14,I16,I9,3I7,1PG17.8)
         IF(IUNITMNW.GT.0.AND.IDINT(FWELL(1,L)).EQ.0) THEN
C        reset layer associated with Q to top layer of multi-layer mnw-well
         do n=1,nwell2
          if(WELL2(8,N).GT.1.0D30.and.WELL2(7,N).LT.1.0D30) then
...
C8F6----ADD FLOW RATE TO BUFFER.
      IF(IUNITMNW.GT.0)THEN
        IF(IWL2CB.NE.0) THEN
          IF(IDINT(FWELL(4,L)).GT.0)
     1    BUFF(IC,IR,IL)=BUFF(IC,IR,IL)+SNGL(QQ)
        ELSE
...
C8F7----SEE IF FLOW IS POSITIVE OR NEGATIVE.
      IF(IDINT(FWELL(4,L)).GT.0) THEN
         IF(QQ.LT.ZERO) GO TO 90
         IF(QQ.EQ.ZERO) GO TO 99
         IF(QQ.GT.ZERO) GO TO 80
      ENDIF
C
C8F8----FLOW RATE IS POSITIVE (RECHARGE). ADD IT TO RATIN (NOTE: MNW-WELLS ADDED IN MNW PACKAGE).
   80 IF(IDINT(FWELL(4,L)).GT.0) RATIN=RATIN+QQ
      GO TO 99
C
C8F9----FLOW RATE IS NEGATIVE (DISCHARGE). ADD IT TO RATOUT (NOTE: MNW-WELLS ADDED IN MNW PACKAGE).
   90 IF(IDINT(FWELL(4,L)).GT.0) RATOUT=RATOUT-QQ
C
...
C       OR IF RETURNING THE FLOW IN THE FARM-WELL ARRAY, COPY FLOW TO FARM-WELL.
   99 IF(IUNITMNW.GT.0)THEN
        IF(IWL2CB.NE.0) THEN
          IF(IDINT(FWELL(4,L)).GT.0.AND.IBD.EQ.2) 
     1             CALL UBDSVB(IFWLCB,NCOL,NROW,IC,IR,IL,SNGL(QQ),
     2                  FWELL(1,L),NFWLVL,NAUX,7,IBOUND,NLAY)
        ELSE
          IF(IBD.EQ.2) CALL UBDSVB(IFWLCB,NCOL,NROW,IC,IR,IL,SNGL(QQ),
     1                  FWELL(1,L),NFWLVL,NAUX,7,IBOUND,NLAY)
        ENDIF
      ELSE
        IF(IBD.EQ.2) CALL UBDSVB(IFWLCB,NCOL,NROW,IC,IR,IL,SNGL(QQ),
     1                  FWELL(1,L),NFWLVL,NAUX,7,IBOUND,NLAY)        
      ENDIF
      IF(IFWLAL.NE.0) FWELL(NFWLVL,L)=QQ
  100 CONTINUE
C
...
C
C
      SUBROUTINE FMP2FNRBD(KSTP,KPER,IGRID)
C-----VERSION 2 09/21/09 FMP2FNRBD
C     ******************************************************************
C     CALCULATE AND PRINT VOLUMETRIC BUDGET FOR FARM NET RECHARGE
C     ******************************************************************
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE FMPMODULE,    ONLY:FNRCH,NFARMS,IFID,IFNRCB
...
C2B5----IF FARM NET RECHARGE IS POSITIVE ADD IT TO RATIN ELSE ADD IT TO RATOUT.
      IF(QQ.LT.ZERO) GOTO 27
      IF(QQ.EQ.ZERO) GOTO 29
      IF(QQ.EQ.ZERO) GOTO 26
   26 RATIN=RATIN+QQ
      GO TO 29
   27 RATOUT=RATOUT-QQ
      GO TO 29
...
C       (FIRST RECORD CONTAINS LAYER NUMBERS; SECOND RECORD CONTAINS FLOW VALUES).
      write(fmt_number,'(i10)')NCOL
      fmt_string = '('//TRIM(ADJUSTL(fmt_number))//'I12)'
      DO I=1,NROW
      WRITE(IOUT,FMT=TRIM(fmt_string)) (IRCH(J,I),J=1,NCOL)
      ENDDO
      write(fmt_number,'(i10)')NCOL
      fmt_string = '('//TRIM(ADJUSTL(fmt_number))//'F12.4)'
      DO I=1,NROW
...
C4A1----WRITE HEADER TO ASCII FILE.
      IF(IFNRCB.EQ.1) THEN
      OPEN(UNIT=1004, FILE='FNRCH_ARRAY.OUT', STATUS='UNKNOWN')
      WRITE(1004,1) TEXT,KPER,KSTP,NCOL,NROW,NLAY,TIME,TIMEUNIT
C
...
C4A2A---WRITE ONE RECORD OF FLOW VALUES IF ONLY ONE LAYER
      ...
      fmt_string = '('//TRIM(ADJUSTL(fmt_number))//'G12.6)'
      DO I=1,NROW
      WRITE(1004,FMT=TRIM(fmt_string))(BUFF(J,I,1),J=1,NCOL)
      ENDDO
      ELSE
C
...
C       (FIRST RECORD CONTAINS LAYER NUMBERS; SECOND RECORD CONTAINS FLOW VALUES).
      ...
      fmt_string = '('//TRIM(ADJUSTL(fmt_number))//'I12)'
      DO I=1,NROW
      WRITE(1004,FMT=TRIM(fmt_string)) (IRCH(J,I),J=1,NCOL)
      ENDDO
      write(fmt_number,'(i10)')NCOL
      fmt_string = '('//TRIM(ADJUSTL(fmt_number))//'F12.4)'
      DO I=1,NROW
      WRITE(1004,FMT=TRIM(fmt_string)) (BUFF(J,I,IRCH(J,I)),J=1,NCOL)
      ENDDO
c 
      ENDIF
C
...
C4B1----WRITE HEADER TO ASCII FILE
      IF(IFNRCB.EQ.2) THEN
      OPEN(UNIT=1003, FILE='FNRCH_LIST.OUT', STATUS='UNKNOWN')
      IF(KPER.EQ.1.AND.KSTP.EQ.1) WRITE(1003,33) TIMEUNIT
   33 FORMAT
...
C4B2----WRITE LIST OF CUMULATIVE FARM NET RECHARGE FOR EACH FARM
        ...
        FNRECH=0.D0         
          ...
          ENDDO
          ENDDO
        WRITE(1003,64) KPER,KSTP,TIME,NF,FNRECH
...
C       (TO BUILD TIME SERIES FOR FARMS: PER,STP, AND TIME ARE INCLUDED FOR EACH FARM)
        IF(IFNRCB.EQ.3) THEN
         OPEN(UNIT=1005, FILE='FNRCH_LIST_BIN.OUT', STATUS='UNKNOWN',
     1   FORM=FORM)
          WRITE(IOUT,65) IFNRCB,KSTP,KPER
   65     FORMAT(/,1X,'SAVING ',
     1    '"TIME SERIES OF CUMULATIVE FARM NET RECHARGE FOR EACH FARM"',
     2    ' ON UNIT',I4,' AT TIME STEP',I3,', STRESS PERIOD',I4)      
          IF(KPER.EQ.1.AND.KSTP.EQ.1) THEN
           WRITE(1005)    '         PER','         STP',TIMEUNIT,
     1      '         FID','        RATE'
          ENDIF
C
C6A1B---HEADER OF LISTS PRINTED AT TIME STEPS THAT ARE SPECIFIED BY OUTPUT CONTROL:
   ...
   66     FORMAT(/,1X,'SAVING ',
     1    '"CUMULATIVE FARM NET RECHARGE FOR EACH FARM" ON UNIT',I4,
     2    ' AT TIME STEP',I3,', STRESS PERIOD',I4)
          WRITE(IFNRCB) KSTP,KPER,TEXT,NCOL,NROW,NLAY
...
C6A2----WRITE LIST OF CUMULATIVE FARM NET RECHARGE FOR EACH FARM
        ...
        FNRECH=0.D0         
          ...
          ENDDO
          IF(IFNRCB.EQ.3) THEN
             WRITE(1005) KPER,KSTP,TIME,NF,FNRECH
          ELSEIF(IFNRCB.GT.3.AND.IBD.EQ.1) THEN
             WRITE(IFNRCB) NF, FNRECH
          ENDIF
        ENDDO
C
C
...
C
C
      SUBROUTINE FMP2INTLSTRD(ILIST,LDIM,NLIST,INPACK,IOUT)          
C-----VERSION 2 09/21/09 FMP2INTLSTRD
C     ******************************************************************
...
C
C
      SUBROUTINE FMP2LSTRD(RLIST,LDIM,NLIST,INPACK,IOUT,ISCLOC1,ISCLOC2)
C-----VERSION 2 09/21/09 FMP2LSTRD
C     ******************************************************************
...
C2===== CHECK FOR SCALE FACTOR ==============================================================================
  ...
  116      FORMAT(1X,'LIST SCALING FACTOR=',1PG12.5)
           IF(ISCLOC1.EQ.0.AND.ISCLOC2.EQ.0) THEN
             WRITE(IOUT,*)'THE SCALE FACTOR DOES NOT APPLY TO ANY FIELD'
              STOP
...
C
C
      SUBROUTINE FMP2DPLSTRD(DPRLIST,LDIM,NLIST,INPACK,IOUT,ISCLOC1,
     1                       ISCLOC2)          
C-----VERSION 2 09/21/09 FMP2DPLSTRD
C     ******************************************************************
...
C2A-----CHECK FOR EXTERNAL LIST OF SCALE FACTORS AND READ NAME OF LIST
         ...
         IF(LINE(ISTART:ISTOP).EQ.'EXTERNAL') THEN
           ...
           CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,I,R,IOUT,IN)
           INSFAC=I
           WRITE(IOUT,120) INSFAC
  120      FORMAT(1X,'READING SCALE FACTOR LIST ON UNIT',I4)
         ELSEIF(LINE(ISTART:ISTOP).EQ.'OPEN/CLOSE') THEN
           ISFACFL=2
           CALL URWORD(LINE,LLOC,ISTART,ISTOP,0,N,R,IOUT,IN)
           SFACNAME=LINE(ISTART:ISTOP)
           INSFAC=100
           WRITE(IOUT,119) INSFAC,SFACNAME
  119      FORMAT(1X,/1X,'OPENING SCALE FACTOR LIST FILE ON UNIT',
     1     I4,':',/1X,A)
           OPEN(UNIT=INSFAC,FILE=SFACNAME,ACTION=ACTION(1))
         ELSE
C
...
C2C-----READ LIST OF SCALE FACTORS
         IF(ISFACFL.GT.0) THEN
           CALL FMP2LSTRD(SLIST,2,LDIM,INSFAC,IOUT,0,0)
           IF(ISFACFL.EQ.2) CLOSE(UNIT=100)
           WRITE(IOUT,117)
...
C
C
      SUBROUTINE FMP2DPWDLSTRD(DPRLIST,WORD,LDIM,NLIST,INPACK,IOUT,
     1                         ISCLOC1,ISCLOC2)          
C-----VERSION 2 09/21/09 FMP2DPWDLSTRD
C     ******************************************************************
C     Read and print a multi-dimensional list of double precision data,and
...
123   if(il.lt.ldim.and.ichar(w).eq.32.or.ichar(w).eq.9) then           !32=blank, 9=tab
      ...
      IF(ICHAR(W).LT.45.OR.ICHAR(W).GT.57) THEN   
...
C
C
      SUBROUTINE FMP2WELRD(NLIST,RLIST,LSTBEG,LDIM,MXLIST,IAL,INPACK,
     1  IOUT,LABEL,CAUX,NCAUX,NAUX,NCOL,NROW,NLAY,ISCLOC1,ISCLOC2,ITERP,
     2  IUNITMNW)
C-----VERSION 2 09/21/09 FMP2WELRD (modified ULSTRD)
C     ******************************************************************
...
C     ------------------------------------------------------------------
     ...
     1  NROW,NLAY,ISCLOC1,ISCLOC2,ITERP,IUNITMNW
...
C3C-----Read the optional values from the line
      IF(NAUX.GT.0) THEN
         ...
         CALL 
     1   URWORDDP(LINE,LLOC,ISTART,ISTOP,4,IDUM,R,RLIST(JJ,II),IOUT,IN)
210      CONTINUE
      END IF
C
C3D-----Write the values that were read.
      NN=II-LSTBEG+1
      IF(ITERP.EQ.1) WRITE(IOUT,205) NN,(IDINT(RLIST(JJ,II)),JJ=1,5),   !modfication of ULSTRD
     1                                  (RLIST(JJ,II),JJ=6,NREAD2)
205   FORMAT(1X,4I7,2I11,6ES16.4)                                       !modfication of ULSTRD
C
C3E-----Check for illegal grid location
      IF(IUNITMNW.EQ.0) THEN
        IF(K.LT.1 .OR. K.GT.NLAY) THEN
         ...
         CALL USTOP(' ')
        END IF
      ELSEIF(IUNITMNW.GT.0) THEN
        IF(K.LT.0 .OR. K.GT.NLAY) THEN                                  !modfication of ULSTRD: ZERO LAYER INDICATES THAT LAYERS ARE DEFINED IN MNW PACKAGE
         WRITE(IOUT,*) ' Layer number in list is outside of the grid'
         CALL USTOP(' ')
        END IF
      ENDIF
...
C
C
      SUBROUTINE FMP2WELPARRD(IN,PACK,IOUTU,PTYP,RLIST,LSTVL,LSTDIM,
     1           NREAD,MXLST,NTOT,IPVL1,IPVL2,LABEL,CAUX,NCAUX,NAUX)
C-----VERSION 2 09/21/09 FMP2WELPARRD (modified UPARLSTSUB)
C     ******************************************************************
...
C4------Check that the parameter is not already active.
 ...
 1030       FORMAT(/,1X,'*** ERROR: PARAMETER "',A,
     ...
     &          ' -- STOP EXECUTION (FMP2WELPARRD)')                    !modfication of UPARLSTSUB
            CALL USTOP(' ')
          ENDIF
C
...
C8A-----value.   
   ...
   86       CONTINUE
            ...
            IR=IDINT(RLIST(2,II))
            IC=IDINT(RLIST(3,II))
            IF(IOUTU.GT.0)WRITE(IOUT,89)                                !modfication of UPARLSTSUB
     1         II,(IDINT(RLIST(JJ,II)),JJ=1,5),(RLIST(JJ,II),JJ=6,NREAD)
   89       FORMAT(1X,4I7,2I11,6ES16.4)                                 !modfication of UPARLSTSUB
   90     CONTINUE
C
...
C     Both ncon and nopv are already defined in FMP2FM.
C
C     Enter ncon=number of constraints = 3+#cells
...
C     ------------------------------------------------------------------
      ...
      USE FMPMODULE, ONLY:OPT,IOPFL,ICID
      USE FMPBLK,    ONLY:FPS
      IMPLICIT NONE
C     ------------------------------------------------------------------
C        ARGUMENTS:
...
C1===== PRINT ORIGINAL FLOW RATES / RESOURCE CONSTRAINTS ====================================================
      IF(IOPFL.EQ.-3.OR.IOPFL.EQ.-2.OR.IOPFL.EQ.3.OR.IOPFL.EQ.2) THEN
        IF(IOPFL.EQ.-3.OR.IOPFL.EQ.-2) IOUTFILE=IOUT
        IF(IOPFL.EQ.3.OR.IOPFL.EQ.2) THEN
          OPEN(UNIT=1006, FILE='ACR_OPT.OUT', STATUS='UNKNOWN')
          IOUTFILE=1006
        ENDIF
        WRITE(IOUTFILE,30) KPER,KSTP,NF
   30   format(//,'ORIGINAL & OPTIMIZED FLOWRATES',7X,
     1  'PERIOD: ',I4,',    STEP:',I4,',    FARM #:',I9,/,84('-'))
C      WRITE(IOUTFILE,31) NCELL,EF,QMXF/EF,ADSW/EF,NRDE/EF,QMXF,ADSW,NRDE
...
C     4'  R-SWD-INI*EF: ',ES9.3,'  NR-SWD-INI*EF: ',ES9.3)
      WRITE(IOUTFILE,31) NCELL,QMXF,ADSW,NRDE
...
C3D-----PRINT TABLEAU MATRIX A
      IF(IOPFL.EQ.-4.OR.IOPFL.EQ.4) THEN
        IF(IOPFL.EQ.-4) IOUTFILE=IOUT
        IF(IOPFL.EQ.4) THEN
          OPEN(UNIT=1006, FILE='ACR_OPT.OUT', STATUS='UNKNOWN')
          IOUTFILE=1006
        ENDIF
        WRITE(IOUTFILE,40) KPER,KSTP,NF
   40   FORMAT(1X,/,1X,'ACREAGE OPTIMIZATION - TABLEAU MATRIX:',7X,
     1  'PERIOD: ',I4,',    STEP:',I4,',    FARM #:',I9,/)
      write(fmt_number,'(i10)')NCELL*3+1
      fmt_string='('//TRIM(ADJUSTL(fmt_number))//'G10.4)'
      DO I=1,NCELL+4
      WRITE(IOUTFILE,FMT=TRIM(fmt_string)) (A(I,J),J=1,NCELL*3+1) 
      ENDDO
      ENDIF
C
...
C
      IF(IOPFL.EQ.-3.OR.IOPFL.EQ.3) THEN
        IF(IOPFL.EQ.-3) IOUTFILE=IOUT
        IF(IOPFL.EQ.3) THEN
          OPEN(UNIT=1006, FILE='ACR_OPT.OUT', STATUS='UNKNOWN')
          IOUTFILE=1006
        ENDIF
      if(j.eq.1) then
      write(IOUTFILE,'(1X,/,"FRACTIONS OF OPTIMIZED CELL AREAS:",/)')
      write(IOUTFILE,60)
...
C6A-----PRINT FLOW RATES / RESOURCE CONSTRAINTS
      IF(IOPFL.EQ.-3.OR.IOPFL.EQ.-2.OR.IOPFL.EQ.3.OR.IOPFL.EQ.2) THEN
        ...
        IF(IOPFL.EQ.3.OR.IOPFL.EQ.2) THEN
          ...
          IOUTFILE=1006
        ENDIF
C      WRITE(IOUTFILE,3) GWSUM,SWSUM,NRSUM,GWSUM/EF,SWSUM/EF,NRSUM/EF
C    3 FORMAT(1X,/,1X,
...
C     4'  R-SWD-FIN:    ',ES9.3,'  NR-SWD-FIN:    ',ES9.3)
      WRITE(IOUTFILE,3) GWSUM,SWSUM,NRSUM
    3 FORMAT(1X,/,1X,
...
C
C
      FUNCTION RTFUNC(FUNC,D,M,N,Y,XACC,J,IC,IR)
C     ******************************************************************
...
C     ------------------------------------------------------------------
      INTEGER J,IC,IR
      DOUBLE PRECISION RTFUNC,FUNC,D,M,N,Y,XACC
      EXTERNAL FUNC      
C     ------------------------------------------------------------------
C        LOCAL VARIABLES:
C     ------------------------------------------------------------------
...
C1===== DEFINE EXCLUSIONS
      ...
      IF(Y.GE.D) RTFUNC=D
      GOTO 12
      ELSEIF(D.GT.1D0.AND.(DABS(D-M).LT.XACC.OR.Y.LE.1D0)) THEN
      RTFUNC=Y
      GOTO 12
      ENDIF
C
C2===== DEFINE LATERAL BOUNDS X1 AND X2
      IF(Y.GE.D) X1 = ((DLOG(Y/D)/DLOG(M/D))**(1/N))*(D-1D0)+1D0 -XACC
      IF(Y.LT.D) X1=1D0
      IF(X1.LT.1D0) X1=1D0
...
C       (REDEFINE ROOT, RETURNED AS RTFUNC, UNTIL ITS ACCURACY IS � XACC)
      DO J=1,JMAX
          ...
          IF(DABS(DX).LT.XACC .OR. FMID.EQ.0D0) GO TO 12
      ENDDO
      WRITE(*,1) -Y,IR,IC
...
C        ARGUMENTS:
C     ------------------------------------------------------------------
      DOUBLE PRECISION FUNC,X,D,M,N,Y
C     ------------------------------------------------------------------
C
...
C        ARGUMENTS:
C     ------------------------------------------------------------------
      INTEGER L,NF,ISTARTFL,KPER,KSTP
...
C1===== PRINT HEADERS =======================================================================================
      IF(IPAPFL.EQ.-1.OR.IPAPFL.EQ.1) THEN
        IF(IPAPFL.EQ.-1) IOUTFILE=IOUT
        IF(IPAPFL.EQ.1) THEN
          OPEN(UNIT=1007, FILE='PRIOR.OUT', STATUS='UNKNOWN')
          IOUTFILE=1007
        ENDIF
C
      WRITE(IOUTFILE,3) NF,KPER,KSTP 
    3 FORMAT(/,1X,'PRIOR APPROPRIATION FOR FARM: ',I7,
     ...
     3 12X,'REACH',3X,'SEGMENT',3X,'SEGMENT')
      WRITE(IOUTFILE,4) NF,L,ISTRM(4,L),IDIVAR(1,ISTRM(4,L))
...
C2E1----CHECK IF THERE ARE SENIOR FARMS DOWNSTREAM THE SAME CANAL AS CURRENT FARM
      ISENDSTR=0
      IFCRIDOLD=0
      DO M=3,NSTRM                                                      !reach loop within reach reach loop of FMP2FM
      DO N=1,NFARMS
...
c                         (2) SEG(2,nseg) in FMP2FM will be set to zero!  --> zero solution!
      IF(RDEL.LT.0D0) THEN
...
c       - subtracted from prevous cumulative PDIV (for TF-ND < 0 & for surplus < flowinmin).
      ...
      IF(IPAPFL.EQ.-1.OR.IPAPFL.EQ.1) THEN
        ...
        IF(IPAPFL.EQ.1) THEN
          ...
          IOUTFILE=1007
        ENDIF
      WRITE(IOUTFILE,7) SIN,SOUT,QSTRMIN(NF),ADIV,PDIV,RDEL,FLOWIN,
     1 FLOWIN,FLOWINMIN(L),FLOWIN-FLOWINMIN(L),STAGE(1)
...
C
        ...
        ELSE                                                            !if no min streamflow in river need to be maintained for higher rank farms on downstream diverting canals
           ...
           STRM(12,L)=SNGL(SFRADD(L)-RDR(l))                            !HAS TO BE DONE HERE ARLEADY: EXITING FMP2FM!
...
C
C2E4B---PRINT BUDGET AFTER CUMULATING PDIV (BUT BEFORE EXITING FMP2FM AND MF2K5 SOLUTION)
        IF(IPAPFL.EQ.-1.OR.IPAPFL.EQ.1) THEN
          IF(IPAPFL.EQ.-1) IOUTFILE=IOUT
          IF(IPAPFL.EQ.1) THEN
            OPEN(UNIT=1007, FILE='PRIOR.OUT', STATUS='UNKNOWN')
            IOUTFILE=1007
          ENDIF
          WRITE(IOUTFILE,9) SIN,SOUT,QSTRMIN(NF),ADIV,PDIV,RDEL,FLOWIN,
     1    FLOWIN-DEL,FLOWINMIN(L),FLOWIN-FLOWINMIN(L),STAGE(2),ACTION(1)
...
C         exceeds actually possible diversion (ADIV).                     --> "exceedance solution!"
     ...
     3  SOUT.NE.0D0.OR.DVRS.EQ.0D0))THEN                                !  |__
        ...
        RDEL=FLOWIN-FLOWINMIN(L)                                        !STRM(12,L)=SFRADD(L)-RDEL IS DONE IN FMP2FM
C
300     ITER=0
...
C3B-----PRINT BUDGET AFTER SOLUTION (BUT BEFORE MF2K5 SOLUTION)
        IF(IPAPFL.EQ.-1.OR.IPAPFL.EQ.1) THEN
          ...
          IF(IPAPFL.EQ.1) THEN
            ...
            IOUTFILE=1007
          ENDIF
          WRITE(IOUTFILE,11) SIN,SOUT,QSTRMIN(NF),ADIV,PDIV,RDELOLD,
     1    FLOWIN,FLOWIN-RDEL,FLOWINMIN(L),RDEL,STAGE(3),ACTION(ISOL)
...
200   IJUNUSTR=0
      IFCRIDALT=0 
      DO M=3,NSTRM                                                      !reach loop within reach loop of FMP2FM
      DO N=1,NFARMS
...
C     WATER-RIGHTS RANKING DRIVEN PRIOR APPROPRIATION SYSTEM OF FARMS (IALLOT=3)
C     ------------------------------------------------------------------
...
C     ------------------------------------------------------------------
        ...
        DEALLOCATE(FMPDAT(IGRID)%IALLOT)
