      MODULE GWFSUBMODULE
        ...
        REAL,   SAVE, DIMENSION(:),   POINTER ::DCOME
        REAL,   SAVE, DIMENSION(:),   POINTER ::DCOMV
        ...
        REAL,   SAVE, DIMENSION(:),   POINTER ::SUBE
        REAL,   SAVE, DIMENSION(:),   POINTER ::SUBV
      ...
      TYPE GWFSUBTYPE
        ...
        REAL,    DIMENSION(:),   POINTER ::DCOME
        REAL,    DIMENSION(:),   POINTER ::DCOMV
        REAL,    DIMENSION(:),   POINTER ::A1
        REAL,    DIMENSION(:),   POINTER ::A2
        REAL,    DIMENSION(:),   POINTER ::BB
        REAL,    DIMENSION(:),   POINTER ::SUB
        REAL,    DIMENSION(:),   POINTER ::SUBE
        REAL,    DIMENSION(:),   POINTER ::SUBV
...
C       CALL U2DREL(RNB(LOC1),ANAME(10),NROW,NCOL,LAYNUM,IN,IOUT)
       CALL U2DREL(BUFF(:,:,1),ANAME(12),NROW,NCOL,LAYNUM,IN,IOUT)
...
C       CALL U2DREL(SUB(LOC1),ANAME(4),NROW,NCOL,K,IN,IOUT)
       ...
       CALL GWF2SUB72D1D(BUFF(:,:,1),NCOL,NROW,SUBE,NND1,LOC1)
       CALL U2DREL(BUFF(:,:,1),ANAME(5),NROW,NCOL,K,IN,IOUT)
       CALL GWF2SUB72D1D(BUFF(:,:,1),NCOL,NROW,SUBV,NND1,LOC1)
  260  CONTINUE
C
...
C       CALL U2DREL(DCOM(LOC1),ANAME(7),NROW,NCOL,K,IN,IOUT)
       ...
       CALL GWF2SUB72D1D(BUFF(:,:,1),NCOL,NROW,DCOME,ND1,LOC1)
       WRITE(IOUT,308) KQ
       CALL U2DREL(BUFF(:,:,1),ANAME(9),NROW,NCOL,K,IN,IOUT)
       CALL GWF2SUB72D1D(BUFF(:,:,1),NCOL,NROW,DCOMV,ND1,LOC1)
       WRITE(IOUT,308) KQ
C       CALL U2DREL(DZ(LOC1),ANAME(8),NROW,NCOL,K,IN,IOUT)
       CALL U2DREL(BUFF(:,:,1),ANAME(10),NROW,NCOL,K,IN,IOUT)
       CALL GWF2SUB72D1D(BUFF(:,:,1),NCOL,NROW,DZ,ND1,LOC1)
       WRITE(IOUT,308) KQ
C       CALL U2DINT(NZ(LOC1),ANAME(9),NROW,NCOL,K,IN,IOUT)
       CALL U2DINT(IBUFF,ANAME(11),NROW,NCOL,K,IN,IOUT)
       L=LOC1-1
       DO 335 I=1,NROW
       DO 335 J=1,NCOL
       L=L+1
...
C8------CALCULATE VOLUME CHANGE IN INTERBED STORAGE FOR TIME STEP.
       ...
       STRGV=0.0
       IF(HHNEW.LT.HHC) STRGV=SCV(LOC2)*(HHC-HHNEW)
       STRGE=STRG-STRGV
C
C9------ACCUMULATE SUBSIDENCE ASSOCIATED WITH CHANGE IN STORAGE
       AREA=DELR(IC)*DELC(IR)
       SUB(LOC2) =SUB(LOC2)+STRG/AREA
C9A----ACCUMULATE ELASTIC AND INELASTIC COMPACTION SEPARATELY (May, 2009)
       SUBV(LOC2)=SUBV(LOC2)+STRGV/AREA
       SUBE(LOC2)=SUBE(LOC2)+STRGE/AREA
C
...
C7A------INCLUDING DELAY AND NO-DELAY INTERBEDS.
      IF(OCFLGS(18,NNSTP).OR.OCFLGS(19,NNSTP)) THEN
       ...
       WRITE(IOUT,76) KQ
        IF(ISBOCF(9).LT.0) CALL ULAPRS(SUBE(LOC2),TEXT(10),KSTP,KPER,
     1            NCOL,NROW,K,-ISBOCF(9),IOUT)
        IF(ISBOCF(9).GE.0) CALL ULAPRW(SUBE(LOC2),TEXT(10),KSTP,KPER,
     1             NCOL,NROW,K,ISBOCF(9),IOUT)
        ENDIF
        IF(OCFLGS(19,NNSTP)) THEN
         CALL ULASAV(SUBE(LOC2),TEXT(11),KSTP,KPER,PERTIM,TOTIM,NCOL,
     1              NROW,KQ,ISBOCU(9))
        ENDIF
  410   CONTINUE
       ENDIF
       IF(NDF) THEN
        ...
        IF(OCFLGS(18,NNSTP)) THEN
        WRITE(IOUT,82) KQ
         IF(ISBOCF(9).LT.0) CALL ULAPRS(DCOME(LOC2),TEXT(11),KSTP,KPER,
     1            NCOL,NROW,K,-ISBOCF(9),IOUT)
         IF(ISBOCF(9).GE.0) CALL ULAPRW(DCOME(LOC2),TEXT(11),KSTP,KPER,
     1             NCOL,NROW,K,ISBOCF(9),IOUT)
        ENDIF
        IF(OCFLGS(19,NNSTP)) THEN
         CALL ULASAV(DCOME(LOC2),TEXT(11),KSTP,KPER,PERTIM,TOTIM,NCOL,
     1              NROW,KQ,ISBOCU(9))
        ENDIF
  420   CONTINUE
       ENDIF
...
C*****WORK HERE... change ocflgs and isbocf numbers
      IF(OCFLGS(20,NNSTP).OR.OCFLGS(21,NNSTP)) THEN
       ...
       WRITE(IOUT,76) KQ
        IF(ISBOCF(10).LT.0) CALL ULAPRS(SUBV(LOC2),TEXT(12),KSTP,KPER,
     1            NCOL,NROW,K,-ISBOCF(10),IOUT)
        IF(ISBOCF(10).GE.0) CALL ULAPRW(SUBV(LOC2),TEXT(12),KSTP,KPER,
     1             NCOL,NROW,K,ISBOCF(10),IOUT)
        ENDIF
        IF(OCFLGS(21,NNSTP)) THEN
         CALL ULASAV(SUBV(LOC2),TEXT(12),KSTP,KPER,PERTIM,TOTIM,NCOL,
     1              NROW,KQ,ISBOCU(10))
        ENDIF
  430   CONTINUE
       ENDIF
       IF(NDF) THEN
        ...
        IF(OCFLGS(20,NNSTP)) THEN
        WRITE(IOUT,82) KQ
         IF(ISBOCF(10).LT.0) CALL ULAPRS(DCOMV(LOC2),TEXT(13),KSTP,KPER,
     1            NCOL,NROW,K,-ISBOCF(10),IOUT)
         IF(ISBOCF(10).GE.0) CALL ULAPRW(DCOMV(LOC2),TEXT(13),KSTP,KPER,
     1             NCOL,NROW,K,ISBOCF(10),IOUT)
        ENDIF
        IF(OCFLGS(21,NNSTP)) THEN
         CALL ULASAV(DCOMV(LOC2),TEXT(13),KSTP,KPER,PERTIM,TOTIM,NCOL,
     1              NROW,KQ,ISBOCU(10))
        ENDIF
  440   CONTINUE
       ENDIF
...
C25-------SUM NO-DELAY INELASTIC COMPACTION IN ALL MODEL LAYERS.
       IF(NNDF) THEN
        ...
        N=N+1
        LOC2=LOCT+N
        BUFF(IC,IR,K)=BUFF(IC,IR,K)+SUBV(LOC2)
        ENDDO
        ENDDO
       ENDDO
       ENDIF
