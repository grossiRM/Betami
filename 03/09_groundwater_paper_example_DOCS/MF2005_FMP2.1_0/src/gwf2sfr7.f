     ...
     +'$Id: gwf2sfr7.f 4535 2009-02-15'
        INTEGER,SAVE,POINTER:: NSS,NSEGDIM,NSTRM,NSFRPAR,ISTCB1,ISTCB2  !NSEGDIM INSERTED BY WSCHMID
      ...
      TYPE GWFSFRTYPE
        INTEGER,     POINTER:: NSS,NSEGDIM,NSTRM,NSFRPAR,ISTCB1,ISTCB2  !NSEGDIM INSERTED BY WSCHMID
...
C     VERSION  7.1.01: February, 2009
C     ******************************************************************
C     SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GWFSFRMODULE
...
C     ------------------------------------------------------------------
      ...
      INTEGER nssar, nstrmar !, nsegdim                                 !NSEGDIM DELETED BY ESCHMID
      REAL r, seglen, sumlen, thsslpe, thislpe, uhcslpe, rchlen, dist
      REAL epsslpe
C     ------------------------------------------------------------------
      iterp = 1
      idum(1) = 0
      ALLOCATE (NSS, NSEGDIM, NSTRM, TOTSPFLOW)                         !NSEGDIM INSERTED BY WSCHMID
...
C1------IDENTIFY PACKAGE AND INITIALIZE NSTRM.
      WRITE (IOUT, 9001) In
 9001 FORMAT (1X, /1X, 'SFR7 -- STREAMFLOW ROUTING PACKAGE, '
     +        ,'VERSION 7.1.01, 02/15/2009', /, 9X, 
...
C3------READ ISFROPT FLAGS WHEN NSTRM IS LESS THAN ZERO.
      IF ( NSTRM.LT.0 ) THEN
        IF ( NSFRPAR.GT.0 ) THEN
          WRITE(IOUT, 9002)
 9002     FORMAT (//, 'NSTRM IS NEGATIVE AND NSFRPAR IS GREATER THAN ',
     +          'ZERO ', /1X , ' ALTERNATE SFR7 OPTIONS DO NOT ',
...
Cdep  changed DSTROT to FXLKOT
      ...
      STROUT = 0.0
      FXLKOT = 0.0
      ALLOCATE (STRM(29,nstrmar), ISTRM(5,nstrmar))
...
C6------PRINT INFORMATION THAT WAS READ.
 ...
 9003 FORMAT (//, ' NUMBER OF STREAM REACHES IS', I5, //, 
     ...
     +        I15, //, ' MAXIMUM ERROR FOR STREAM LEAKAGE RATES IS',    !WSCHMID: INCREASED I5 TO I15
...
!      END IF
     ...
     +                 STRM(6, ii), THTS(ii), THTI(ii), EPS(ii), UHC(ii)
        ...
        END IF
        IF ( IUZT.EQ.1 ) THEN
          IF ( STRT(jrch, irch, krch).LE.BOTM(jrch, irch, krch) )
...
!              THTR(ii) = THTS(ii) - SC2NWT(jrch, irch, krch)
!     +                   /(DELR(jrch)*DELC(irch))
            ELSE IF ( Iunitbcf.GT.0 ) THEN
              IF ( LAYCON(krch).EQ.1 ) THEN
...
C     ------------------------------------------------------------------
C
!      IF ( Iunitlpf.GT.0 .OR. Iunithuf.GT.0 .OR. Iunitnwt.GT.0 ) THEN 
      IF ( Iunitlpf.GT.0 .OR. Iunithuf.GT.0 ) THEN 
!        IF ( ISFROPT.EQ.2.OR.ISFROPT.EQ.4 )
!     +     CALL SGWF2SFR7UHC(Iunitlpf, Iunithuf, Iunitnwt)
        IF ( ISFROPT.EQ.2.OR.ISFROPT.EQ.4 )
...
!      SUBROUTINE SGWF2SFR7UHC(Iunitlpf, Iunithuf, Iunitnwt)
...
C     ******************************************************************
      ...
      USE GLOBAL,       ONLY: IOUT, IBOUND, NLAY, NROW, NCOL
      USE GWFLPFMODULE, ONLY: LAYVKA, LAYTYP, VKA, HK
!      USE GWFNWTMODULE, ONLY: LAYVKANWT=>LAYVKA, VKANWT=>VKA, HKNWT=>HK
...
!        ELSEIF ( Iunitnwt.GT.0 ) THEN
!          IF ( IBOUND(jrck, irck, krck).GT.0 ) THEN
!            IF ( LAYVKANWT(krck).EQ.0 ) THEN
!              UHC(ichk) = VKANWT(jrck, irck, krck)
!            ELSE
!              UHC(ichk) = VKANWT(jrck, irck, krck)*
!     +                    HKNWT(jrck, irck, krck)
!            END IF
!          END IF
        ELSE
          IF ( IBOUND(jrck, irck, krck).GT.0 ) THEN
...
C-------SUBROUTINE GWF2SFR7RP
      SUBROUTINE GWF2SFR7RP(In, Iunitgwt, Iunitlak, Kkper, Nsol, Iouts,
     +                      Igrid)
C     ******************************************************************
...
C     ARGUMENTS
C     ------------------------------------------------------------------
      INTEGER Kkper, In, Iunitgwt, Iunitlak, Nsol, Iouts, Igrid
C     ------------------------------------------------------------------
C     LOCAL VARIABLES
C     ------------------------------------------------------------------
...
C9------READ DATA ACCORDING TO VARIABLE ISFROPT.
     ...
     +         isoptflg = 1
          ...
          ELSE IF ( IDIVAR(2, nseg).EQ.-2 ) THEN
            ...
            END IF
          END IF
C9A-----CHECK FOR ERROR WHEN USING "SLOPE"-OPTION, IRDFLG=2
          IF(ABS(IRDFLG.EQ.2).AND.IDIVAR(1,NSEG).GT.0.                          ! WSCHMID 03/17/06
     +       AND.(NSEG.EQ.NSS.OR.NSEG.EQ.1) ) THEN                              ! WSCHMID 01/30/06
          WRITE(IOUT, 9000)                                                     ! WSCHMID 01/30/06
 9000	      FORMAT (//1X, '***ERROR***  A DIVERSION SEGMENT CANNOT BE ',        ! WSCHMID 01/30/06
     +          'THE FIRST OR LAST SEGMENT, IF IRDFLG = 2',/1X,                 ! WSCHMID 01/30/06
     +          'PROGRAM IS STOPPING')                                          ! WSCHMID 01/30/06
            CALL USTOP(' ')                                                     ! WSCHMID 01/30/06
          ENDIF                                                                 ! WSCHMID 01/30/06
        END DO
...
C
 ...
 9026 FORMAT (//)
C
C25-----COMPUTE STREAMBED ELEVATIONS AND STREAMBED SLOPE BY INTERPOLATING DEPTHS BETWEEN 
C       THE ELEVATIONS OF THE LAND SURFACE AND THE TOP OF THE STREAMBED SPECIFIED
C       AS SPECFIED IN THE SFR INPUT FILE FOR THE UPSTREAM AND DOWNSTREAM ENDS OF A DIVERSION SEGMENT
C
C       C25 INSERTED BY WOLFGANG SCHMID (FOR GENERAL USE - NOT ONLY FOR USE WITH FARM PROCESS):
C       FOR DIVERSION SEGMENTS IN AREAS OF UNDULATING MORPHOLOGY, WHERE THE DEFAULT INTERPOLATION 
C       OF THE TOP OF THE STREAMBED WOULD CREATE STREAMBED ELEVATIONS ABOVE THE LAND SURFACE.
C
C25A----CHECK FOR LAST REACH IN DIVERSION SEGMENT AND DETERMINE DOWNSTREAM DIFFERENCE BETWEEN
C       GROUND SURFACE ELEVATION AND TOP OF STREAM BED ELEVATION
        ...
        DO irch = 2, NSTRM
          IF( ISTRM(4, irch).GT.1 ) THEN
            IF( IDIVAR(1,ISTRM(4, irch)-1).GT.0 ) THEN
              icp = ISTRM(3, irch-1)
              irp = ISTRM(2, irch-1)
              IF( ISTRM(5, irch).EQ.1 )  SEG(13, ISTRM(4, irch)-1) =            !WSCHMID: REUSING SEG(13,)  NOT USED ANYMORE FURTHER DOWN ==> REUSEDF FOR 'SLOPE' OPTION, IRDFLG=2!
...
C
C26-----COMPUTE STREAMBED TOP ELEVATION FOR REACHES OF DIVERSION SEGMENTS 
C       (OVERWRITE DEFAULT STREAMBED TOP ELEVATION)
            seglen = SEG(1, nseg)
            sumlen = 0.0
            DO irch = 1, NSTRM
              IF( IDIVAR(1, ISTRM(4, irch)).EQ.IDIVAR(1, nseg) ) THEN
                icalc = ISEG(1, nseg)  !rsr, icalc needs a value                !wschmid: icalc already defined in C18
                rchlen = STRM(1, irch)
                dist = sumlen + (0.5 * rchlen)
                sumlen = sumlen + rchlen
                ic = ISTRM(3, irch)
                ir = ISTRM(2, irch)
                updiff = 0.0   !rsr, added to make sure updiff has a value      !wschmid: updiff is calculated for every first reach of segment (i.e., there is no segment with less than one reach)
     ...
     +                  SEG(12, nseg)) / seglen) * dist)
            ...
            END DO
C
C27-----COMPUTE STREAMBED SLOPE FOR REACHES OF DIVERSION SEGMENTS.
C       NOTE THAT FIRST AND LAST REACH CAN NOT BE DIVERSION-SEGMENT REACHES.
            DO irch = 2, NSTRM-1                                                !1ST AND LAST REACH CAN'T AND NEVER WILL BE CANAL REACHES!!
              IF( IDIVAR(1, ISTRM(4, irch)).EQ.IDIVAR(1, nseg) ) THEN
     ...
     +                             STRM(1, irch-1) + STRM(1, irch))
                END IF
C28-----EXCLUSIONS: DO NOT ALLOW NEGATIVE SLOPES, BUT ALLOW ZERO OR CLOSE TO ZERO SLOPE
C       AND ADJUST TO MINIMUM SLOPE
                IF(Strm(2, irch).LT.0D0) THEN
                  WRITE(IOUT,9027)
 9027             FORMAT(1X,
     +           'ERROR: SLOPE IN DIVERSION SEGMENT CANNOT BE NEGATIVE:'
     +            ,/,1X,'INCREASE STREAMBED ELEVATION AT UPSTREAM END',
     +            ' OF SEGMENT, OR',/,1X,'DECREASE STREAMBED ELEVATION',
     +            ' AT DOWNSTREAM END OF SEGMENT!')
                  STOP
                END IF
                IF( Strm(2, irch ).LT.1.0E-7) THEN
                  WRITE(IOUT,9028)  Istrm(4,irch), Istrm(5,irch),
     +                              Strm(2, irch)
 9028             FORMAT(1X,'SLOPE FOR SEGMENT AND REACH ',2(1x,I5),
     +                   ' IS WITH ',E10.3,' LESS THAN 1.0E-07:',
     +                   ' SETTING SLOPE TO 1.0E-06 ')
                  Strm(2, irch) = 1.0E-06
                END IF
              END IF
            END DO
...
C31B-----SEARCH FOR UPPER MOST ACTIVE CELL IN STREAM REACH.
          ilay = il   !rsr, moved before IF to be sure ilay has a value
...
!      USE GWFNWTMODULE, ONLY: Drhs, Dhcof, Heps
      IMPLICIT NONE
...
C     ARGUMENTS
C     -----------------------------------------------------------------
      INTEGER Kkiter, Kkper, Iunitlak, Igrid, Kkstp, Nlakesar
      DOUBLE PRECISION Theta, Vol(Nlakesar)
...
C     -----------------------------------------------------------------
     ...
     +                 thr, thet1, dvrsn, hdiff, rhsh1, hcofh1, rhsh2, 
     +                 hcofh2
!rsr  DOUBLE PRECISION grad
            ...
            IF( istsg.GT.1 )THEN
              DO kss = 2, NSS
                ...
                IF( IDIVAR(1,kss).EQ.istsg-1 ) THEN
                   dvrsn = SEG(2,idivseg)
                   iprior = IDIVAR(2,kss)
                  CALL GWF2SFR7DIVERS(iprior, idivseg, upflw, dvrsn)
                  DVRSFLW(kss) = dvrsn
...
C30d----BEGIN LOOP FOR NEWTON SOLVER IF ACTIVE (Hardwired inactive until NWT released)
!          DO ii = 1, idr
!           SUMRCH(l) = 0.0
!            SUMLEAK(l) = 0.0
!            IF ( ii.EQ.2 ) THEN
...
C47-----CALCULATE FLOBOT1 AND FLOBOT2 WHEN ICALC GREATER THAN 1.
     ...
     +                        fbcheck, NWAVST(:,l), maxwav, FOLDFLBT(l))
                  flobot2 = CALCUNSATFLOBOT(depth2, avhc, fks,
...
C RGN 10/3/07 added check for flwmpt to avoid NaN values.
...
C67-----ROUTE FLOW DOWN CHANNEL WHEN ACTIVE.
     ...
     +               istsg, nreach, itstr, qlat, flobot, width, l)
            ...
            END IF
            STRM(28,l) = qc
!            IF ( qc-flobot.LT.NEARZERO ) THEN
!              flobot = cstr*(hdiff)
!              IF ( 0.5*flobot.GT.qd ) THEN
!                flobot = qd
!                qd = 0.0
!                flowc = flobot
!              ELSE
!                qd = qd - 0.5*flobot
!                IF (flowc.LT.flobot)flowc = flobot
!              END IF
!            END IF
            STRM(27,l) = qd
            flowot = qd
...
!          Drhs(ic,ir,il) = Drhs(ic,ir,il) +  
!     +           (rhsh2 - rhsh1)/Heps
!          Dhcof(ic,ir,il) = Dhcof(ic,ir,il) +  
!     +           (hcofh2 - hcofh1)/Heps
!        END IF
        END DO
C        
...
C     *****************************************************************
      USE GWFSFRMODULE
      USE GWFSFRBLK,    ONLY: CLOSEZERO, NEARZERO
...
C     ------------------------------------------------------------------
      ...
      INTEGER Igrid, Nlakesar, Nsol
      DOUBLE PRECISION    Vol(Nlakesar)
C     ------------------------------------------------------------------
C     LOCAL VARIABLES
C     ------------------------------------------------------------------
     ...
     +        icalccheck, iss, lsub, irt, itstr, imassroute
...
C9------COMPUTE INFLOW OF A STREAM SEGMENT EMANATING FROM A LAKE.
 ...
 9001           FORMAT (/5X, '*** WARNING *** NEGATIVE LAKE DIVERSION ',
     +                  'NOT ALLOWED; SEG = ', I6, /10X, 
     +                  'SFR7BD CODE WILL ASSUME FLOW = 0.0'/)
                SEG(2, istsg) = 0.0
                FXLKOT(istsg) = SEG(2, istsg)
              END IF
C
C10-----SPECIFIED FLOW FROM LAKE IS ZERO AND ICALC IS ZERO.
              ...
              END IF
C
C11-----OUTFLOW FROM LAKE COMPUTED IN LAKE PACKAGE
              IF ( FXLKOT(istsg).LE.CLOSEZERO ) THEN
                flowin = STROUT(istsg)
            ...
            IF ( flobot.GE.0.0D0 ) THEN
              ...
              IF ( runof.LT.NEARZERO ) THEN
                IF ( flowin+runoff+precip.LT.NEARZERO ) THEN
                  runof = 0.0D0
                  etstr = 0.0D0
C-WSCHMID--changed abs(runof) back to runof to accommodate negative runoff = farm diversion in FMP (05/07/08)                   
...
C-WSCHMID--changed abs(runof) back to runof to accommodate negative runoff = farm diversion in FMP (05/07/08) 
     ...
     +                NEARZERO ) THEN
          ...
          IF ( flobot.GE.flow ) THEN
            ...
            IF ( icalc.EQ.0 ) depth = 0.0D0
          END IF
          IF ( icalccheck.EQ.1 .AND. h.LT.sbot ) THEN
            flobot = CALCUNSATFLOBOT(depth, avhc, fks, wetperm,
     +                   sbdthk, areamax, strlen, fbcheck, NWAVST(:,l),
     +                   maxwav, FOLDFLBT(l))
          END IF
...
cDEP   need to fix for unsaturated flow
          IF ( imassroute.EQ.1 .AND. ibd.EQ.2 )
...
C4------ICALC EQUALS 2.
     ...
     +                        roughbnk, slope, wetperm2, 
     +                        flwdlk2, width2)
              DLKOTFLW(lk, istsg) = (flwdlk1-flwdlk2)/(dlkstr1-dlkstr2)
...
C     ARGUMENTS
C     ------------------------------------------------------------------
      INTEGER Iprior, Idivseg       !rsr: Idivseg is unused
...
C     VERSION  7.1.01: February 15, 2009
C     ******************************************************************
      USE GWFSFRMODULE, ONLY: SFRUZBD
      USE GWFSFRBLK,    ONLY: CLOSEZERO
...
C     VERSION  7.1.01: February 15, 2009
C     ******************************************************************
      USE GWFSFRMODULE, ONLY: CONST, XSEC
      USE GWFSFRBLK,    ONLY: NEARZERO
      USE GLOBAL,       ONLY: IOUT
      IMPLICIT NONE
...
C-------FUNCTION CALCUNSATFLOBOT written by RGN, MAY 24, 2004
     ...
     +                              Maxwav, Foldflbt)
      USE GWFSFRMODULE, ONLY: NSTRAIL, ISUZN
      USE GWFSFRBLK
      IMPLICIT NONE
C     ------------------------------------------------------------------
C     SPECIFICATIONS:
...
C     ******************************************************************
     ...
     +                        SUMLEAK,SUMRCH
      USE GWFSFRBLK,    ONLY: NEARZERO, CLOSEZERO
...
C         STORAGE WHEN WATER TABLE RISES TO ELEVATION OF STREAMBED.
     ...
     +                   Uzspst, Uzflst, Uzdpst, Uzthst, Deltinc,kkper, 
     +                   kkstp)
        iset = 1
        DO kk = 1, iuznhold
          loop(kk) = 0
...
C     ******************************************************************
C
      USE GWFSFRMODULE, ONLY: NSTOTRL, ISUZN, STRM
      USE GWFSFRBLK,    ONLY: CLOSEZERO
      USE GLOBAL,       ONLY: IOUT
      IMPLICIT NONE
C     ------------------------------------------------------------------
...
C     LOCAL VARIABLES
C     ------------------------------------------------------------------
      INTEGER i, ick, iset, iwav, numwaves, iuzntemp, ntotuzn, kkper,
     +        kkstp
...
C     LOCAL VARIABLES
C     ------------------------------------------------------------------
      INTEGER i, ick, iset, numwaves, iuzntemp, kkper, kkstp
      DOUBLE PRECISION dlength, zoldist, totflux, surflux, oldsflx
C     -----------------------------------------------------------------
C
C1------ROUTE WAVES THROUGH EACH UNSATURATED ZONE COMPARTMENT BENEATH
...
C     VERSION  7.1.01: February 15, 2009
C     ******************************************************************
      USE GWFSFRMODULE, ONLY: NSTOTRL, NSFRSETS, NSTRAIL
      USE GWFSFRBLK,    ONLY: THETAB, FLUXB
      USE GLOBAL,    ONLY: IOUT
      IMPLICIT NONE
      INTRINSIC ABS, DABS
C     ------------------------------------------------------------------
...
C6------CALCULATE SHORTEST TIME FOR WAVE INTERCEPTION.
        ...
        DO j = Numwaves, 3, -1
          IF ( checktime(j).LE.shortest ) THEN
            ...
            DO k = j + 1, Numwaves
              IF ( ABS(checktime(k)-checktime(j)).GT.CLOSEZERO ) 
     +            more(k) = 0
            END DO
          END IF
        END DO
...
C     VERSION  7.1.01: February 15, 2009
C     ******************************************************************
      USE GWFSFRMODULE, ONLY: NSTOTRL, NSTRAIL, NSFRSETS
      USE GWFSFRBLK
      USE GLOBAL,       ONLY: IOUT
      IMPLICIT NONE
      INTRINSIC FLOAT
...
C       AND CALCULATE EQUATION OF EACH LINE.
     ...
     +                   ' check input. Segment number ', Istsg
            slope = 0.0  !rsr, slope needs a value
...
C     VERSION  7.1.01: February 15, 2009
C***********************************************************************
      USE GWFSFRMODULE, ONLY: ISEG, WEIGHT, SEG, FLWTOL
      USE GLOBAL,       ONLY: IOUT 
      USE GWFSFRBLK
      IMPLICIT NONE
C     ------------------------------------------------------------------
...
C     ------------------------------------------------------------------
     ...
     +                 Qd, Awdth, Bwdth
      REAL Qcnst, Strlen, Deltinc, Qlat
      INTRINSIC ABS, DABS
C     ------------------------------------------------------------------
C     LOCAL VARIABLES
C     ------------------------------------------------------------------
...
C8------EXIT LOOP IF delq LESS THAN TOLERANCE.
        ...
        END IF
          i = i + 1
      END DO CONVERGE
!      IF ( Qc+Qlat+(Qa-Qb)-flobot.LT.NEARZERO .AND. 
!     +      flobot.GT.0.0D0 ) Qd = 0.0
...
C     ------------------------------------------------------------------
      ...
      DEALLOCATE (GWFSFRDAT(IGRID)%NSEGDIM) !NSEGDIM INSERTED BY WSCHMID
...
C     ------------------------------------------------------------------
      ...
      NSEGDIM=>GWFSFRDAT(IGRID)%NSEGDIM  !NSEGDIM INSERTED BY WSCHMID
...
C     ------------------------------------------------------------------
      ...
      GWFSFRDAT(IGRID)%NSEGDIM=>NSEGDIM  !NSEGDIM INSERTED BY WSCHMID
