...
# 
# Recall that, for all its sophistication, formal statistical parameter estimation software does the same things as a modeler does during manual trial-and-error.  It (1) assigns best guess parameter values, (2) runs the model, (3) compares model outputs to measured values, (4) if model-to-measurment fit is inadequate, it adjusts parameter values and repeats the process. Once model-to-measurement fit ceases to improve, it stops. All of these steps must be automated. All of the decision criteria must also be quantified and are recorded, making the process transparent and reproducible.
# 
# Here is a flow chart of manual trial-and-error history matching from *Applied Groundwater Modeling (2nd edition)* by [Anderson et al. (2015)](https://www.sciencedirect.com/book/9780120581030/applied-groundwater-modeling): 
# 
...
display(Image(url='./pest_flow1.gif'))


# ## The PEST Input Dataset
# 
# PEST and PEST++ are called "universal" or "model-independent" inverse codes because they can be bolted on the outside of any model.  Well, that is, any model they can talk to and run, which means the model needs to meet these two criteria:
# 
...
# 
# You should see six lines, each with a file name pair. They should look something like this:
# 
# >```
# >* model input/output
